...
#ifdef CCAM
    real, dimension(mp) :: tmp1, tmp2, tmp3, tmp4, tmp5
#endif
...
#ifdef CCAM
    tmp1 = met%tvair-CTfrz
    CALL qsatfjh(mp, qstvair, CRMH2o, Crmair, CTETENA, CTETENB, CTETENC, tmp1, met%pmb)
#else
  ...
  clai_thresh, Csboltz, Cemsoil, Cemleaf, Ccapp &
    ...
    DO iter = 1, NITER
       ...
       CALL comp_friction_vel(canopy%us, iter, mp, CVONK, CUMIN, CPI_C,      &
                             canopy%zetar(:,iter), rough%zref_uv, rough%zref_tq,     &
                             rough%z0m, met%ua )
#else
       CALL comp_friction_vel(canopy%us, iter, mp, CVONK, CUMIN, CPI_C,      &
...
#ifdef CCAM
       tmp1 = ssnow%tss-CTfrz
       CALL qsatfjh(mp, ssnow%qstss, CRMH2o, Crmair, CTETENA, CTETENB, CTETENC,tmp1,met%pmb)
#else
      ...
      !H!        call pore_space_relative_humidity(ssnow,soil,veg)
       ...
       IF (cable_user%soil_struc=='default') THEN
          ...
          ELSE !by default assumes Humidity Deficit Method
             ...
             tmp3 = REAL(ssnow%satfrac)
             tmp4 = REAL(ssnow%rtevap_sat)
             tmp5 = REAL(ssnow%rtevap_unsat)
             ssnow%potev = Humidity_deficit_method( mp, Ctfrz, tmp1,cable_user%or_evap,     &
                                 cable_user%gw_model, cable_user%litter,       &
                                 air%rho, air%rlam,           & 
                                 dq,dq_unsat,ssnow%qstss,   & 
                                 tmp2,      &
                                 ssnow%isflag, tmp3,ssnow%rtsoil, &
                                 tmp4,  tmp5, & 
...
write(6,*) "SLI is not an option right now"
       ...
       CALL update_zetar( mp, iterplus, NITER, canopy%zetar, iter, nrb, CVONK, &
                     ...
                     canopy%fh, canopy%fe, canopy%fhs, tmp1 ) 
#else
       CALL update_zetar( mp, iterplus, NITER, canopy%zetar, iter, nrb, CVONK, &
                     CGRAV, CCAPP, CLAI_THRESH, CZETmul, CZETPOS, CZETNEG,     &
                     cable_user%soil_struc, air%rho, met%tk,  met%fsd, &
...
canopy%gswx_T(:)    = Surf_conductance(:)               !fill CABLE type for now
    ...
    tmp3 = canopy%zetar(:,NITER)*0.1*rough%z0m/rough%zref_tq
    canopy%cdtq = canopy%cduv * &
                    ( LOG( rough%zref_uv / rough%z0m) -              &
                      psim( tmp1, mp, CPI_C )   &
                     + psim( tmp2, mp, CPI_C ) & ! new term from Ian Harman
                    )                                          &
