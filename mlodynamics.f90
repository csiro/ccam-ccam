...
integer, parameter :: intmeth=1    ! interpolation method (0=bi-linear, 1=bi-cubic)
...
do k=1,wlev
  ...
  !sig=dep(1:ifull,k)/dd(1:ifull) ! sig will be the same for each index iq
...
! interpolating density profiles.  The grid pivoting has been
...
real, dimension(ifull) :: nip,ipn,ipe,ips,ipw,ipmax,imu,imv
...
real, dimension(ifull,wlev) :: w_u,w_v,w_t,w_s,nw,dzbar,dum,dz
...
rho=rho-rho0
call bounds(rho)
rhobar(1:ifull,1)=rho(1:ifull,1)*dsig(1)
do ii=2,wlev
  rhobar(1:ifull,ii)=rhobar(1:ifull,ii-1)+rho(1:ifull,ii)*dsig(ii)
end do
do ii=1,wlev
  rhobar(1:ifull,ii)=rhobar(1:ifull,ii)/sigh(ii)
end do
call bounds(rhobar,corner=.true.)
...
call stagtruedelta(rhobar,dep,wtr,drhobardxu,drhobardyu,drhobardxv,drhobardyv) ! staggered at time t
!do ii=1,wlev
!  rhou=0.5*(rho(1:ifull,ii)+rho(ie,ii))
!  rhov=0.5*(rho(1:ifull,ii)+rho(in,ii))
!  tnu=0.5*( rhobar(in,ii)+grav*rhou*((1.-sig(ii))*neta(in) -sig(ii)*dd(in) ) &
!          +rhobar(ine,ii)+grav*rhou*((1.-sig(ii))*neta(ine)-sig(ii)*dd(ine)))
!  tsu=0.5*( rhobar(is,ii)+grav*rhou*((1.-sig(ii))*neta(is) -sig(ii)*dd(is) ) &
!          +rhobar(ise,ii)+grav*rhou*((1.-sig(ii))*neta(ise)-sig(ii)*dd(ise)))
!  tev=0.5*( rhobar(ie,ii)+grav*rhov*((1.-sig(ii))*neta(ie) -sig(ii)*dd(ie) ) &
!          +rhobar(ien,ii)+grav*rhov*((1.-sig(ii))*neta(ien)-sig(ii)*dd(ien)))
!  twv=0.5*( rhobar(iw,ii)+grav*rhov*((1.-sig(ii))*neta(iw) -sig(ii)*dd(iw) ) &
!          +rhobar(iwn,ii)+grav*rhov*((1.-sig(ii))*neta(iwn)-sig(ii)*dd(iwn)))
!  drhobardxu(:,ii)=(rhobar(ie,ii)     +grav*rhou*((1.-sig(ii))*neta(ie)     -sig(ii)*dd(ie))       & 
!                   -rhobar(1:ifull,ii)-grav*rhou*((1.-sig(ii))*neta(1:ifull)-sig(ii)*dd(1:ifull))) &
!                   *emu(1:ifull)/ds
!  drhobardyu(:,ii)=stwgt(:,1)*0.5*(tnu-tsu)*emu(1:ifull)/ds
!  drhobardxv(:,ii)=stwgt(:,2)*0.5*(tev-twv)*emv(1:ifull)/ds
!  drhobardyv(:,ii)=(rhobar(in,ii)     +grav*rhov*((1.-sig(ii))*neta(in)     -sig(ii)*dd(in))       & 
!                   -rhobar(1:ifull,ii)-grav*rhov*((1.-sig(ii))*neta(1:ifull)-sig(ii)*dd(1:ifull))) &
!                   *emv(1:ifull)/ds
!end do
...
call boundsuv(cou,cov)
odum=neta(1:ifull)+dd(1:ifull)
div=(cou(1:ifull,1)/emu(1:ifull)-cou(iwu,1)/emu(iwu)+cov(1:ifull,1)/emv(1:ifull)-cov(isv,1)/emv(isv)) &
    *em(1:ifull)*em(1:ifull)/ds
div=div+nw(:,1)/(dsig(1)*odum) ! nw is at half levels
mps(1:ifull,1)=neta(1:ifull)/odum-(1.-eps)*0.5*dt*div
do ii=2,wlev
  div=(cou(1:ifull,ii)/emu(1:ifull)-cou(iwu,ii)/emu(iwu)+cov(1:ifull,ii)/emv(1:ifull)-cov(isv,ii)/emv(isv)) &
      *em(1:ifull)*em(1:ifull)/ds
...
select case(intmeth)
  ...
  case default
    call mloblintsb(mps,nface,xg,yg,wtr)
end select
...
! Prepare free surface gradient terms for sea ice (used later)
uiu(:,1)=grav*(neta(ie)-neta(1:ifull))*emu(1:ifull)/ds ! staggered
uiv(:,1)=grav*(neta(in)-neta(1:ifull))*emv(1:ifull)/ds
call mlounstaguv(uiu,uiv,tau(:,1),tav(:,1),ee) ! trick from JLM
snu(1:ifull)=niu(1:ifull)+(1.-eps)*0.5*dt*(f(1:ifull)*niv(1:ifull)-tau(:,1))
snv(1:ifull)=niv(1:ifull)-(1.-eps)*0.5*dt*(f(1:ifull)*niu(1:ifull)+tav(:,1))
...
select case(intmeth)
  ...
  case default
    call mloblints(cou,nface,xg,yg,wtr)
    call mloblints(cov,nface,xg,yg,wtr)
    call mloblints(cow,nface,xg,yg,wtr)
end select
...
odum=1./(1.+(1.+eps)*(1.+eps)*0.25*dt*dt*f(1:ifull)*f(1:ifull))
do ii=1,wlev
  uau(:,ii)=uau(:,ii)*odum ! unstaggered
  uav(:,ii)=uav(:,ii)*odum
end do
tau(:,1)=-(1.+eps)*0.5*dt*odum
tav(:,1)=-(1.+eps)*0.5*dt*odum
tau(:,3)= (1.+eps)*0.5*dt*f(1:ifull)*tau(:,1)
tav(:,3)=-(1.+eps)*0.5*dt*f(1:ifull)*tav(:,1)
call mlostaguv(uau,uav,cou(1:ifull,:),cov(1:ifull,:),ee)
call mlostaguv(tau(:,1:1),tav(:,1:1),tau(:,2:2),tav(:,2:2),ee)
call mlostaguv(tau(:,3:3),tav(:,3:3),tau(:,4:4),tav(:,4:4),ee)
do ii=1,wlev
  rhou=0.5*(rho(1:ifull,ii)+rho(ie,ii))+rho0
  rhov=0.5*(rho(1:ifull,ii)+rho(in,ii))+rho0
  rhobaru=0.5*(rhobar(1:ifull,ii)+rhobar(ie,ii))+rho0
  ...
  if (usetide.eq.1) then
    ...
    kku(:,ii)=au+bu*(dpsdxu+grav*rhou*dttdxu+grav*drhobardxu(:,ii)*sig(ii)*ddu) &
                +cu*(dpsdyu+grav*rhou*dttdyu+grav*drhobardyu(:,ii)*sig(ii)*ddu)

    llv(:,ii)=(bv*drhobardyv(:,ii)+cv*drhobardxv(:,ii))*grav*sig(ii)
    mmv(:,ii)=bv*grav*(rhobarv+rhov*(1.-sal))
    nnv(:,ii)=cv*grav*(rhobarv+rhov*(1.-sal))
    kkv(:,ii)=av+bv*(dpsdyv+grav*rhov*dttdyv+grav*drhobardyv(:,ii)*sig(ii)*ddv) &
                +cv*(dpsdxv+grav*rhov*dttdxv+grav*drhobardxv(:,ii)*sig(ii)*ddv)
  else
    llu(:,ii)=(bu*drhobardxu(:,ii)+cu*drhobardyu(:,ii))*grav*sig(ii)
    mmu(:,ii)=bu*grav*rhobaru
    nnu(:,ii)=cu*grav*rhobaru
    kku(:,ii)=au+bu*(dpsdxu+grav*drhobardxu(:,ii)*sig(ii)*ddu) &
                +cu*(dpsdyu+grav*drhobardyu(:,ii)*sig(ii)*ddu)

    llv(:,ii)=(bv*drhobardyv(:,ii)+cv*drhobardxv(:,ii))*grav*sig(ii)
    mmv(:,ii)=bv*grav*rhobarv
    nnv(:,ii)=cv*grav*rhobarv
    kkv(:,ii)=av+bv*(dpsdyv+grav*drhobardyv(:,ii)*sig(ii)*ddv) &
                +cv*(dpsdxv+grav*drhobardxv(:,ii)*sig(ii)*ddv)
  end if
...
dsnudeta =spu(1:ifull)*0.5-squ(1:ifull)*emu(1:ifull)/ds
dsnuwdeta=spu(iwu)*0.5    +squ(iwu)*emu(iwu)/ds
dsnvdeta =spv(1:ifull)*0.5-sqv(1:ifull)*emv(1:ifull)/ds
dsnvsdeta=spv(isv)*0.5    +sqv(isv)*emv(isv)/ds
...
do ll=1,llmax
  ...
  snu(1:ifull)=sou(1:ifull)+spu(1:ifull)*oeu+squ(1:ifull)*detadxu+sru(1:ifull)*detadyu
  snv(1:ifull)=sov(1:ifull)+spv(1:ifull)*oev+sqv(1:ifull)*detadyv+srv(1:ifull)*detadxv
  call boundsuv(snu,snv)
  sdiv=(snu(1:ifull)/emu(1:ifull)-snu(iwu)/emu(iwu)  &
       +snv(1:ifull)/emv(1:ifull)-snv(isv)/emv(isv)) &
       *em(1:ifull)*em(1:ifull)/ds
  ! Update neta using d(div)/d(neta) to improve stability
  ddivdeta=(dsnudeta/emu(1:ifull)-dsnuwdeta/emu(iwu)  &
           +dsnvdeta/emv(1:ifull)-dsnvsdeta/emv(isv)) &
           *em(1:ifull)*em(1:ifull)*odum/ds           &
           +sdiv
  sdiv=sdiv*odum

  ! For now, assume Boussinesq fluid and treat density in continuity equation as constant
  seta=-neta(1:ifull)+(xps-(1.+eps)*0.5*dt*sdiv)/(1.+(1.+eps)*0.5*dt*ddivdeta) ! implicit in first term of Taylor expansion
  !seta=-neta(1:ifull)+(xps-0.5*sdiv*dd(1:ifull)/odum)/(1.+0.5*dt*sdiv/odum)
...
call mlounstaguv(tau(:,1:1),tav(:,1:1),siu,siv,ee)
odum=1./(1.+(1.+eps)*(1.+eps)*0.25*dt*dt*f(1:ifull)*f(1:ifull))
uiu(:,1)=uiu(:,1)-(1.+eps)*0.5*dt*siu(:,1)-(1.+eps)*(1.+eps)*0.25*dt*dt*f(1:ifull)*f(1:ifull)*siv(:,1) ! unstaggered
uiv(:,1)=uiv(:,1)-(1.+eps)*0.5*dt*siv(:,1)+(1.+eps)*(1.+eps)*0.25*dt*dt*f(1:ifull)*f(1:ifull)*siu(:,1)
uiu(:,1)=uiu(:,1)*odum
uiv(:,1)=uiv(:,1)*odum
! note missing 0.5 as ip is the average of t and t+1
tau(:,1)=-dt*odum
tav(:,1)=-dt*odum
tau(:,3)= dt*f(1:ifull)*tau(:,1)
tav(:,3)=-dt*f(1:ifull)*tav(:,1)
call mlostaguv(uiu,uiv,siu,siv,ee)
call mlostaguv(tau(:,1:1),tav(:,1:1),tau(:,2:2),tav(:,2:2),ee)
call mlostaguv(tau(:,3:3),tav(:,3:3),tau(:,4:4),tav(:,4:4),ee)
niu(1:ifull)=siu(:,1) ! staggered
niv(1:ifull)=siv(:,1)
...
ibu(1:ifull)=tau(:,2)/imu
ibv(1:ifull)=tav(:,2)/imv
icu(1:ifull)=tau(:,4)/imu
icv(1:ifull)=tav(:,4)/imv
...
do ll=1,llmax
  ...
  call bounds(ip,corner=.true.)
  tnu=0.5*(ip(in)+ip(ine))
  tsu=0.5*(ip(is)+ip(ise))
  tev=0.5*(ip(ie)+ip(ien))
  twv=0.5*(ip(iw)+ip(iwn))
...
dipdyv=(ip(in)-ip(1:ifull))*emv(1:ifull)/ds
niu(1:ifull)=(niu(1:ifull)+ibu(1:ifull)*dipdxu+icu(1:ifull)*dipdyu)*ee(1:ifull)*ee(ie)
niv(1:ifull)=(niv(1:ifull)+ibv(1:ifull)*dipdyv+icv(1:ifull)*dipdxv)*ee(1:ifull)*ee(in)
! average ice velocity between t and t+1
...
! implicit first order in time to improve stability

! Horizontal advection for ice area
ndum(1:ifull)=fracice/(em(1:ifull)*em(1:ifull)) ! ndum is an area
call bounds(ndum)
odum=ndum(1:ifull)
odum=odum+0.5*(spu(iwu)*(ndum(1:ifull)+ndum(iw))-abs(spu(iwu))*(ndum(1:ifull)-ndum(iw)))*emu(iwu)/(ds/dt+abs(spu(iwu))*emu(iwu))
odum=odum-0.5*(spu(1:ifull)*(ndum(1:ifull)+ndum(ie))+abs(spu(1:ifull))*(ndum(1:ifull)-ndum(ie)))*emu(1:ifull)/(ds/dt+abs(spu(1:ifull))*emu(1:ifull))
odum=odum+0.5*(spv(isv)*(ndum(1:ifull)+ndum(is))-abs(spv(isv))*(ndum(1:ifull)-ndum(is)))*emv(isv)/(ds/dt+abs(spv(isv))*emv(isv))
odum=odum-0.5*(spv(1:ifull)*(ndum(1:ifull)+ndum(in))+abs(spv(1:ifull))*(ndum(1:ifull)-ndum(in)))*emv(1:ifull)/(ds/dt+abs(spv(1:ifull))*emv(1:ifull))
ndum(1:ifull)=odum
ndum=max(ndum,0.)
...
select case(intmeth)
  ...
  case default
    do n=1,nguess
      temp(1:ifull,:) = uc
      call mloblints(temp,nface,xg,yg,wtr)
      do ii=1,kx
        x3d(:,ii) = x - 0.5*(uc(:,ii)+temp(1:ifull,ii)) ! n+1 guess
      end do
      temp(1:ifull,:) = vc
...
! This version is bi-linear for velocity
subroutine mloblints(s,nface,xg,yg,wtr)
...
real, dimension(ifull,size(nface,2)), intent(in) :: xg,yg
real, dimension(ifull+iextra,size(nface,2)), intent(inout) :: s
real, dimension(0:ipan+1,0:jpan+1,1:npan,size(nface,2)) :: sx
...
do n=1,npan         ! first simple copy into larger array
  ...
  do i=1,ipan
    ...
    sx(i,jpan+1,n,:) = s(in(ind(i,jpan,n)),:)
  enddo            ! i loop
  sx(0,0,n,:)           = 0.5*(s(lsw(n),:)+s(lws(n),:))
  sx(0,jpan+1,n,:)      = 0.5*(s(lnw(n),:)+s(iwn(ind(1,jpan,n)),:))
  sx(ipan+1,0,n,:)      = 0.5*(s(ise(ind(ipan,1,n)),:)+s(les(n),:))
  sx(ipan+1,jpan+1,n,:) = 0.5*(s(ine(ind(ipan,jpan,n)),:)+s(ien(ind(ipan,jpan,n)),:))
enddo              ! n loop
...
return
end subroutine mloblints

! This version is bi-linear for scalars
subroutine mloblintsb(s,nface,xg,yg,wtr)
...
do iproc=0,nproc-1
  ...
  do iq=1,drlen(iproc)
    ...
    call lfill(sc,sextra(iproc)%a(iq))
    aad=sc(1,1)-sc(0,1)-sc(1,0)+sc(0,0)
    aab=sc(1,0)-sc(0,0)
    aac=sc(0,1)-sc(0,0)
    sextra(iproc)%a(iq)=aab*xxg+aac*yyg+aad*xxg*yyg+sc(0,0)
...
do iq=1,ifull
  if (wtr(iq)) then
    ...
    do ii=1,wlev
      ...
      call searchdelta(isin ,ddn ,ddu,sx ,idnu,snu)
      call searchdelta(isine,ddne,ddu,sx ,idne,sne)
      call searchdelta(isis ,dds ,ddu,sx ,ids ,ss)
      call searchdelta(isise,ddse,ddu,sx ,idse,sse)
      
      call searchdelta(isx  ,ddx ,ddv,sx ,idv ,sv)
      call searchdelta(isin ,ddn ,ddv,sx ,idnv,snv)
      call searchdelta(isie ,dde ,ddv,sx ,idev,sev)
      call searchdelta(isien,dden,ddv,sx ,iden,sen)
