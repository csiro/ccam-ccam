...
!     N.B. nevapcc option has been removed   
     ...
     .       ,ntest,ntr,nums
      real Aev,Asb,alfcon,alfq,alfs,cfraclim
     .    ,convmax,convtimx,deltaq,delq_av,delt_av
     .    ,den1,den2,den3,detr,dprec,dqrx,dskm
     .    ,entrainp,entrr,entrradd,facuv,fldownn,fluxup
     .    ,frac,fraca,fracb,gam,hbas,hbase,heatlev
     .    ,pwater,pwater0,qavg,qavgb,qbas,qentrr,qprec,qsk,rkmid
     .    ,savg,savgb,sbas,sentrr,sum,tm,totprec,veldt
...
!     could reinstate nbase=0 & kbsav_b from .f0406
      ...
      real alfss(kl),alfm(kl)
      real alfqq_l(kl),alfqq_s(kl),alfqq1_l(kl),alfqq1_s(kl)
      real dsk(kl),qbass(kl),h0(kl),q0(kl),t0(kl)  
...
!     data convshal/ifull*0./
      ...
      do k=1,kl
       ...
       alfss(k)=1.-alfm(k)
       if(nalfs==0)alfss(k)=1.  ! for no alf effect  - usual setting
      enddo     ! k loop
      if(alflnd<1.)then     
        if(alflnd>0.)then  ! common alfsea, alflnd method
          do k=1,kl
...
!       following uses convtime_eff(hrs)=convtime(=~60 km)/dx(in km)  
!       N.B. can give very small factr for small dt &  large dx
        factr(:)=dt/max(dt,convtime*3.6e6*em(1:ifull)/ds) ! to re-scale convpsav
      elseif(convtime<0.)then         
!       convtimx decreases during convection, after it starts    
        do iq=1,ifull 
         convtimx=max(abs(convtime)-(timeconv(iq)-mdelay/3600.),0.)
         factr(iq)=dt/max(dt,convtimx*3600.) 
        enddo
...
c       calculate hs
     ...
     .             (1000.*qq(idjd,k),k=1,kl)
        ...
        enddo
        write (6,"('qbas ',12f7.3/(5x,12f7.3))") 1000.*qbass(:)
        write (6,"('tt   ',12f7.2/(5x,12f7.2))") tt(idjd,:)
        write (6,"('s/cp ',12f7.2/(5x,12f7.2))") s(idjd,:)/cp
        write (6,"('h/cp ',12f7.2/(5x,12f7.2))") 
     .             s(idjd,:)/cp+hlcp*qq(idjd,:)
        write (6,"('hb/cp',12f7.2/(5x,12f7.2))") 
     .             s(idjd,:)/cp+hlcp*qbass(:)
        write (6,"('hs/cp',12f7.2/(5x,12f7.2))") hs(idjd,:)/cp
        write (6,"('cfracc',12f7.3/6x,12f7.3)") cfrac(idjd,:)
        write (6,"('k   ',12i7/(4x,12i7))") (k,k=1,kl)
        write (6,"('es     ',9f7.1/(7x,9f7.1))") es(idjd,:)
        write (6,"('dqsdt6p',6p9f7.1/(7x,9f7.1))") dqsdt(idjd,:)
      endif
...
!        cfrac has both stratif & conv from previous timestep	
         do iq=1,ifull
          if(cfrac(iq,k)<.01*mbase)kdown(iq)=0
         enddo
       endif  ! (mbase>0)
       if(mbase<0)then
!        cfrac has both stratif & conv from previous timestep	
         do iq=1,ifull
          dskm=.001*ds/em(iq)
!         mbase=-80 gives .9, .8, .6, .2, .1 for 8, 16, 32, 64, 72 km
!         mbase=-40 gives .9, .8, .6, .2, .1 for 4,  8, 16, 32, 36 km
!         mbase=-16 gives .9, .8, .5, .2, .1 for 2,  4,  8, 16, 32, 36 km
!          cfraclim=max(1.+dskm/mbase,.1)
!        superseded by
!         mbase=-22 gives  .78, .462, .275, .1 for 4, 8, 16, 62 km
          cfraclim=min(1.,max(.1,-.1*mbase/sqrt(sqrt(dskm*dskm**2))))
          if(cfrac(iq,k)<cfraclim)kdown(iq)=0
!         next line added 13/1/06 because of cfrac problems in newrain
c          if(cfrac(iq,k)<cfraclim.
c     &                     and.qq(iq,k)<.95*qs(iq,k))kdown(iq)=0
         enddo
       endif  ! (mbase<0)
       do iq=1,ifull
...
!     subtract contrib to cloud base layer (unit flux this one)
      do iq=1,ifull
       delq(iq,kb_sav(iq))=-qbase(iq)
       dels(iq,kb_sav(iq))=-sbase(iq)
      enddo
...
c      if(itn>1)then  ! added April 04
c        do iq=1,ifull
c	  if(rnrtc(iq)>0..and.kb_sav(iq)==kbsav(iq).
c     .                      and.kt_sav(iq)==ktsav(iq))then
...
c         do iq=1,ifull
c          if(kt_sav(iq)<kl.and.k<kt_sav(iq))then 
c!            use lowest such layer, e.g. with convrh=.4 
c  	     if(qq(iq,k)/qs(iq,k)<convrh)kt_sav(iq)=k  
c          endif
c         enddo    ! iq loop
c        enddo     ! k loop
c      endif       ! (convrh>0.)
...
!      delq(iq,kt_sav(iq))=delq(iq,kt_sav(iq))+qsk  
       ...
       dels(iq,kt_sav(iq))=entrradd*sbase(iq) ! s flux
...
!      calculate emergent downdraft properties
     ...
     &              dels(idjd,1:kt_sav(idjd))
        write (6,"('delqa',3p9f8.3/(5x,9f8.3))")
     &              delq(idjd,1:kt_sav(idjd))
      endif
...
!      N.B. pre convpsav & dsk; methdetr only used if detrainx>0.
     ...
     &             (dels(idjd,k)+hl*delq(idjd,k),k=1,kl)
...
!     calculate actual delq and dels
      do k=1,kl-1    
       do iq=1,ifull
        ...
        elseif(k==kb_sav(iq))then  ! s and qg only from kb layer
          delq(iq,k)=delq(iq,k)/dsk(k)
          dels(iq,k)=dels(iq,k)/dsk(k)
...
!      if(kt_sav(iq)<kl)then 
       if(kb_sav(iq)<kl)then   ! from Oct 03, same answers but more effic.
!        prevents withdrawal of unrealistally large moisture	
...
c     .                -alfqarr(iq)*hl*delq(iq,kb_sav(iq)) )==0.)then
...
c         endif 
     ...
     .           detr,sig(kb_sav(iq))-sig(kt_sav(iq)),qxcess(iq)
        write(6,"('flux_dsk,fluxq,fluxb,convpsav',5f9.5)")
     .             flux_dsk(iq),fluxq(idjd),fluxb(idjd),convpsav(idjd)
        write(6,"('fluxt',9f8.3/(5x,9f8.3))") fluxt(iq,1:kt_sav(iq))
        write(6,"('delQ*dsk6p',6p9f8.3/(10x,9f8.3))")
     .    (convpsav(idjd)*delq(idjd,k)*dsk(k),k=1,kl)
...
c          if(nums<20)then
     ...
     .   (convpsav(idjd)*delq(idjd,k),k=1,kl)
...
!     endif     ! (shaltime>0.)	   
      ...
      do iq=1,ifull
       if(ktsav(iq)==kl.and.convpsav(iq)>0.)then
         kbsav(iq)=kb_sav(iq) 
         ktsav(iq)=kt_sav(iq)  
       endif  ! (ktsav(iq)==kl.and.convpsav(iq)>0.)
      enddo   ! iq loop
...
!     N.B. convpsav has been updated here with convfact, but without factr term  
      ...
      elseif(methprec==9)then           ! moisten all cloud layers
       do iq=1,ifull  
        ...
        enddo  
       enddo   ! iq loop
      elseif(methprec==3)then           ! moisten middle third
       do k=kuocb+1,kl-1 
        do iq=1,ifull  
...
c        if(iq==idjd)print *,'iq,kb_sav,kt_sav,sum,rkmid ',
...
!           put flux of gas into top convective layer
      ...
      if((ntest>0.or.diag).and.mydiag)then
        write (6,"('uuc ',12f6.1/(4x,12f6.1))") (u(idjd,k),k=1,kl)
        write (6,"('vvc ',12f6.1/(4x,12f6.1))") (v(idjd,k),k=1,kl)
      endif

      if(nmaxpr==1.and.mydiag)then
        write (6,"('ktau,itn,kb_sav,kt_sav,timeconv,factr,'
     &  'flux_dsk,convpsav ',i5,3i3,2f5.2,2f8.5)")
     &   ktau,itn,kb_sav(idjd),kt_sav(idjd),
     &   timeconv(idjd),factr(idjd),flux_dsk(idjd),convpsav(idjd)
      endif
...
!     here the rainfall rate rnrt has been converted to g/m**2/sec
     ...
     .             (tt(idjd,k),k=1,kl)
      ...
      if(nevapls==5)then ! even newer UKMO Thu  03-05-1998
        rKa=2.4e-2
        Dva=2.21
...
8     qg(1:ifull,:)=qq(1:ifull,:)                   
     ...
     .             flux_dsk(idjd),fluxq(idjd),fluxb(idjd),convpsav(idjd)
...
!       set ksctop for shallow convection
     ...
     .       5f8.3)")sigkscb,sigksct,tied_con,tied_over,tied_rh
       ...
       if(ksc==79)then    ! resembles ksc=99
