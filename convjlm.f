...
!     nevapls:  turn off/on ls evap - through parm.h; 0 off, 5 newer UK
     ...
     &  (mbase<0.and.mbase/=-10.and.mbase/=-19.and.mbase/=-23).or.
     &  (nbase<=0.and.nbase/=-4.and.(nbase>-11.or.nbase<-13)))then
        ...
        enddo
        if(mydiag)write(6,*)'klon23,klon2,k500,k935,k972',
...
c        briefly mbase=+9 had no wetfac test; briefly 0 same as -06
         do iq=1,ifull
          es(iq,1)=establ(tt(iq,1))
          pk=ps(iq)*sig(1)
          qs(iq,1)=.622*es(iq,1)/max(pk-es(iq,1),1.)  
          k=kkbb(iq)
           if(fg(iq)>mbase)alfqarr(iq)=alfqarr(iq)*                 !  N.B. qs check done later with qbass
...
c         else
c          do iq=1,ifull
c            sum=ds/(em(iq)*208498.)
c            alfqarrx(iq)=1.+(alfqarr(iq)-1.) *sqrt(sum)  ! early eqn
c          enddo
c         endif
          alfqarr(:)=alfqarrx(:)   ! general default, allowing for ds effects above
          endif
        if(nproc==1)then
          sum=0.
          do iq=1,ifull
           sum=max(sum,alfqarrx(iq))
          enddo
          print *,'max_alfqarrx:',sum
        endif  ! (nproc==1)
...
!         now choose base layer to also have local max of splume+hl*qplume      
     ...
     &           (splume(iq,kb)+hl*qplume(iq,kb))/cp,hs(iq,k)/cp
       ...
       enddo     ! ******************** k loop ****************************
      else !  nbase=-11, -12 and everything except -4
         do iq=1,ifull
...
c            for dsig (.1,.2,.3,.4,.5,.6) 2nd term is (1.09,.59, .38, .26 .19,  .14)
        ...
        if(nmaxpr==1.and.mydiag)print *,'kb_sd,kt_sd,entr,timeconva',
...
c           if(mydiag.and.iq==idjd)print *,'k,sigs,fluxt',k,kb_sav(iq),
c     &           kt_sav(iq),sig(k),sig(kb_sav(iq)),sig(kt_sav(iq)),
c     &    .9*sig(kb_sav(iq))+.1*sig(kt_sav(iq)),fluxt(iq,k),convpsav(iq)
          if(sig(k)<sig(kb_sav(iq)) 
     &       -rhcv*(sig(kb_sav(iq))-sig(kt_sav(iq))).and.                   !  typically rhcv=.1
     &       fluxt(iq,k)<convpsav(iq))then  
c         if(fluxt(iq,k)<convpsav(iq))then  
...
!     - usually with methprec=7 or 8 too
      if(methdetr==1)then  
        do iq=1,ifull     
         detrainn=min(1.,max(.15,               ! thicknesses .1 (full) and .6 (.15)
     &           1.17-1.7*(sig(kb_sav(iq))-sig(kt_sav(iq)))))
         qxcess(iq)=detrainn*rnrtcn(iq) 
        enddo
      endif   ! (methdetr==1)     
      if(methdetr==2)then  ! 09/11  NB methdetr=1 & 2 ignore detrain  
        do iq=1,ifull     
         detrainn=min(1.,max(.16,               ! thicknesses .2 (full) and .6 (.15)
...
c        diff=0, .14, .2,.3, .4, .6, .7 gives detrainn=1, 1, .71, .39, .22, .16, .15, .15   for .15  
         qxcess(iq)=detrainn*rnrtcn(iq) 
        enddo
      endif   ! (methdetr==3)     
...
c        diff=0, .14, .2, .3, .4, .5, .6, .7 gives detrainn=1, 1., 1., .71, .39, .22, .16, .15   for .15  
c        diff=0, .14, .2, .3, .4, .5, .6, .7 gives detrainn=1, 1., 1., .69, .35, .17, .11, .1     for .1  
         qxcess(iq)=detrainn*rnrtcn(iq) 
        enddo
      endif   ! (methdetr==4)     
...
c        diff=0, .14, .2, .3, .4, .5, .6, .7 gives detrainn=1, 1., 1., .71, .39, .22, .16, .15   for .15 X 
c        diff=0, .14, .2, .3, .4, .5, .6, .7 gives detrainn=1, 1., 1., .69, .35, .17, .11, .1     for .1  X
         qxcess(iq)=detrainn*rnrtcn(iq) 
        enddo
      endif   ! (methdetr==5)     
...
c        diff=0, .14, .2, .3, .4, .5, .6, .7 gives detrainn=1, 1., 1., .69, .35, .17, .11, .1     for .1  X
     ...
     &              rdry*dsig(k)/(cp*sig(k))            
         ...
         enddo
        enddo
        if(nmaxpr==1.and.mydiag)write(6,*) 'timeconvb',timeconv(idjd)
        do iq=1,ifull
     ...
     &    timeconv(idjd),convpsav(idjd)
      endif    ! (itn==1)   ! moved up before factr on 7/3/14
...
c         options to define factr so it is small for (longer-lived) shallow clouds       
     ...
     &                        (sig(kb_sav(iq))-sig(kt_sav(iq)))/.6)  ! -2
         ...
         if(nint(convtime)==-23)then  ! -23. was a common setting
          do iq=1,ifull     ! 30 mins for .6; 40m for .45; 60m for .3; 120m for .15
     ...
     &                        (sig(kb_sav(iq))-sig(kt_sav(iq)))/.6)  ! -24
          ...
          if(nint(convtime)==-26)then  
          do iq=1,ifull     ! 60 mins for .6; 80m for .45; 120m for .3; 240m for .15
     ...
     &                        (sig(kb_sav(iq))-sig(kt_sav(iq)))/.6)  ! -24
          ...
          if(tied_over<0)then  ! e.g. -10 giving .55 for 20 km
           do iq=1,ifull
            sum=ds/(em(iq)*208498.)
            factr(iq)=factr(iq)*(1.-tied_over)*sum/(1.-tied_over*sum)
           enddo
        endif  ! (tied_over<0)
c     endif    ! (itn==1)
...
!            put flux of aerosol into upper layer
     ...
     &     pblh(iq),fldow(iq),tdown(iq),qdown(iq),fluxq(iq)
       write(6,"('ktau,kkbb,wetfac,alfqarr,alqarrx,dtsol,omega8
     &,omgtst8',i5,i3,4f6.3,8pf9.3,f7.3)") ktau,kkbb(idjd),
     &      wetfac(idjd),alfqarr(idjd),alfqarrx(idjd),dtsol,
...
c    N.B. condx used to update rnd03,...,rnd24. LS added in leoncld.f       
