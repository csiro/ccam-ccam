...
!------------------------------------------------------------------------------
      ...
      type blocked_data_1ld
        logical, dimension(:), allocatable :: data
      end type blocked_data_1ld
      type blocked_data_1id
        integer, dimension(:), allocatable :: data
      end type blocked_data_1id
      ...
      type(blocked_data_1ld), dimension(:), allocatable, save :: b_land

      type(blocked_data_1id), dimension(:), allocatable, save :: b_kbsav
      type(blocked_data_1id), dimension(:), allocatable, save :: b_ktsav

      type(blocked_data_1d), dimension(:), allocatable, save :: b_alfin
      type(blocked_data_1d), dimension(:), allocatable, save :: b_bcwd
      type(blocked_data_1d), dimension(:), allocatable, save :: b_cape
      type(blocked_data_1d), dimension(:), allocatable, save :: b_condc
      type(blocked_data_1d), dimension(:), allocatable, save :: b_condg
      type(blocked_data_1d), dimension(:), allocatable, save :: b_conds
      type(blocked_data_1d), dimension(:), allocatable, save :: b_condx
      type(blocked_data_1d), dimension(:), allocatable, save :: b_convpsav
      type(blocked_data_1d), dimension(:), allocatable, save :: b_dustwd
      type(blocked_data_1d), dimension(:), allocatable, save :: b_ocwd
      type(blocked_data_1d), dimension(:), allocatable, save :: b_so2wd
      type(blocked_data_1d), dimension(:), allocatable, save :: b_so4wd
      type(blocked_data_1d), dimension(:), allocatable, save :: b_em
      type(blocked_data_1d), dimension(:), allocatable, save :: b_entrainn
      type(blocked_data_1d), dimension(:), allocatable, save :: b_fg
      type(blocked_data_1d), dimension(:), allocatable, save :: b_pblh
      type(blocked_data_1d), dimension(:), allocatable, save :: b_precc
      type(blocked_data_1d), dimension(:), allocatable, save :: b_precip
      type(blocked_data_1d), dimension(:), allocatable, save :: b_ps
      type(blocked_data_1d), dimension(:), allocatable, save :: b_sgsave
      type(blocked_data_1d), dimension(:), allocatable, save :: b_timeconv
      type(blocked_data_1d), dimension(:), allocatable, save :: b_wetfac

      type(blocked_data_2d), dimension(:), allocatable, save :: b_cfrac
      type(blocked_data_2d), dimension(:), allocatable, save :: b_fluxtot
      type(blocked_data_2d), dimension(:), allocatable, save :: b_dpsldt
      ...
      type(blocked_data_2d), dimension(:), allocatable, save :: b_qg
      type(blocked_data_2d), dimension(:), allocatable, save :: b_qfg
      type(blocked_data_2d), dimension(:), allocatable, save :: b_qlg

      type(blocked_data_3d), dimension(:), allocatable, save :: b_xtg
      type(blocked_data_3d), dimension(:), allocatable, save :: b_tr
     ...
     .    ,sumb
      ...
      allocate(b_conds(nb))
      allocate(b_condx(nb))
      allocate(b_convpsav(nb))
      allocate(b_dpsldt(nb))
      allocate(b_dustwd(nb))
      allocate(b_em(nb))
      allocate(b_entrainn(nb))
      allocate(b_fg(nb))
      allocate(b_fluxtot(nb))
      allocate(b_kbsav(nb))
      allocate(b_ktsav(nb))
      allocate(b_land(nb))
      allocate(b_ocwd(nb))
      ...
      allocate(b_precc(nb))
      allocate(b_precip(nb))
      allocate(b_ps(nb))
      allocate(b_qfg(nb))
      allocate(b_qg(nb))
      allocate(b_qlg(nb))
      allocate(b_sgsave(nb))
      allocate(b_so2wd(nb))
      allocate(b_so4wd(nb))
      allocate(b_t(nb))
      allocate(b_timeconv(nb))
      allocate(b_tr(nb))
      allocate(b_u(nb))
      allocate(b_v(nb))
      allocate(b_wetfac(nb))
      allocate(b_xtg(nb))

      do i=1,nb
        allocate(b_land(i)%data(imax))

        allocate(b_kbsav(i)%data(imax))
        allocate(b_ktsav(i)%data(imax))

        allocate(b_alfin(i)%data(imax))
        allocate(b_bcwd(i)%data(imax))
        allocate(b_cape(i)%data(imax))
        allocate(b_condc(i)%data(imax))
        allocate(b_condg(i)%data(imax))
        allocate(b_conds(i)%data(imax))
        allocate(b_condx(i)%data(imax))
        allocate(b_convpsav(i)%data(imax))
        allocate(b_dustwd(i)%data(imax))
        allocate(b_em(i)%data(imax))
        allocate(b_entrainn(i)%data(imax))
        allocate(b_fg(i)%data(imax))
        allocate(b_ocwd(i)%data(imax))
        allocate(b_pblh(i)%data(imax))
        allocate(b_precc(i)%data(imax))
        allocate(b_precip(i)%data(imax))
        allocate(b_ps(i)%data(imax))
        allocate(b_sgsave(i)%data(imax))
        allocate(b_so2wd(i)%data(imax))
        allocate(b_so4wd(i)%data(imax))
        allocate(b_timeconv(i)%data(imax))
        allocate(b_wetfac(i)%data(imax))

        allocate(b_cfrac(i)%data(imax,kl))
        allocate(b_fluxtot(i)%data(imax,kl))
        allocate(b_dpsldt(i)%data(imax,kl))
        allocate(b_phi_nh(i)%data(imax,kl))
        allocate(b_t(i)%data(imax,kl))
        allocate(b_u(i)%data(imax,kl))
        allocate(b_v(i)%data(imax,kl))
        allocate(b_qg(i)%data(imax,kl))
        allocate(b_qfg(i)%data(imax,kl))
        allocate(b_qlg(i)%data(imax,kl))

        allocate(b_xtg(i)%data(imax,kl,naero))
        allocate(b_tr(i)%data(imax,kl,ntracmax))
      end do
...
!           tied_over=10 gives factor [1, .917, .786, .611, .306] for ds = [200, 100, 50, 25, 8} km     
     ...
     &                   convjlm_begin,convjlm_end
      ...
      use kuocomb_m, only : kbsav,ktsav,convpsav,fluxtot
      use aerosolldr, only : bcwd,dustwd,ocwd,so2wd,so4wd,xtg
      use prec_m, only : cape,precc,precip
      use morepbl_m, only : condc,condg,conds,condx,fg,pblh
      use map_m, only : em
      use arrays_m, only : ps,t,u,v,qg
      use extraout_m, only : sgsave
      use work2_m, only : wetfac
      use cfrac_m, only : cfrac
      use vvel_m, only : dpsldt
      use nharrs_m, only : phi_nh
      use liqwpar_m, only : qfg,qlg
      ...
      do i=1,nb
        ...
        b_alfin(i)%data=alfin(is:ie)
        b_bcwd(i)%data=bcwd(is:ie)
        b_cape(i)%data=cape(is:ie)
        b_condc(i)%data=condc(is:ie)
        b_condg(i)%data=condg(is:ie)
        b_conds(i)%data=conds(is:ie)
        b_condx(i)%data=condx(is:ie)
        b_convpsav(i)%data=convpsav(is:ie)
        b_dustwd(i)%data=dustwd(is:ie)
        b_em(i)%data=em(is:ie)
        b_entrainn(i)%data=entrainn(is:ie)
        b_fg(i)%data=fg(is:ie)
        b_ocwd(i)%data=ocwd(is:ie)
        b_pblh(i)%data=pblh(is:ie)
        b_precc(i)%data=precc(is:ie)
        b_precip(i)%data=precip(is:ie)
        b_ps(i)%data=ps(is:ie)
        b_sgsave(i)%data=sgsave(is:ie)
        b_so2wd(i)%data=so2wd(is:ie)
        b_so4wd(i)%data=so4wd(is:ie)
        b_timeconv(i)%data=timeconv(is:ie)
        b_wetfac(i)%data=wetfac(is:ie)

        b_cfrac(i)%data=cfrac(is:ie,:)
        b_fluxtot(i)%data=fluxtot(is:ie,:)
        b_dpsldt(i)%data=dpsldt(is:ie,:)
...
!$omp parallel do
      ...
      do i=1,nb
        is=(i-1)*imax+1
        ie=i*imax
        kbsav(is:ie)=b_kbsav(i)%data
        ktsav(is:ie)=b_ktsav(i)%data
        bcwd(is:ie)=b_bcwd(i)%data
        cape(is:ie)=b_cape(i)%data
        condc(is:ie)=b_condc(i)%data
        condg(is:ie)=b_condg(i)%data
        conds(is:ie)=b_conds(i)%data
        condx(is:ie)=b_condx(i)%data
        convpsav(is:ie)=b_convpsav(i)%data
        dustwd(is:ie)=b_dustwd(i)%data
        ocwd(is:ie)=b_ocwd(i)%data
        precc(is:ie)=b_precc(i)%data
        precip(is:ie)=b_precip(i)%data
        so2wd(is:ie)=b_so2wd(i)%data
        so4wd(is:ie)=b_so4wd(i)%data
        timeconv(is:ie)=b_timeconv(i)%data

        fluxtot(is:ie,:)=b_fluxtot(i)%data
        t(is:ie,:)=b_t(i)%data
        u(is:ie,:)=b_u(i)%data
        v(is:ie,:)=b_v(i)%data
      ...
      subroutine convjlm_work(tile)     ! jlm convective scheme
...
!     nevapls:  turn off/on ls evap - through parm.h; 0 off, 5 newer UK
      ...
      alfqarr(:)=b_alfin(tile)%data(:)
      omega(1:imax)=b_dpsldt(tile)%data(:,komega)
       
      tt(1:imax,:)=b_t(tile)%data(:,:)       
      qq(1:imax,:)=b_qg(tile)%data(:,:)      
      ...
      do k=2,kl
       ...
       enddo     ! iq loop
      enddo      ! k  loop
      if(nh/=0)phi(:,:)=phi(:,:)+b_phi_nh(tile)%data(:,:)  ! add non-hydrostatic component - MJT
...
!          (middle of k-1 level, uppermost level below pblh)
          if(phi(iq,k-1)<b_pblh(tile)%data(iq)*grav)kkbb(iq)=k-1
         enddo    ! iq loop
        enddo     ! k loop	 
       endif  ! (nbase==-12)
...
!          (uppermost layer, with approx. bottom of layer below pblh)
           if(.5*(phi(iq,k-1)+phi(iq,k))<b_pblh(tile)%data(iq)*grav)kkbb(iq)=k
          enddo    ! iq loop
         enddo     ! k loop
        ...
        if(mbase==-10)then ! fg; qg1   
         do iq=1,imax
          k=kkbb(iq)
           if(b_fg(tile)%data(iq)>0.)alfqarr(iq)=alfqarr(iq)*                 !  mbase=-10; N.B. qs check done later with qbass
     &               max(b_qg(tile)%data(iq,1),b_qg(tile)%data(iq,k980),b_qg(tile)%data(iq,k))/b_qg(tile)%data(iq,k) ! MJT suggestion
         enddo 
        endif  ! (mbase-=-10)
...
c         N.B. if fg<=0, then alfqarr will keep its input value, e.g. 1.25         
         if(b_fg(tile)%data(iq)>0.)alfqarr(iq)=alfqarr(iq)*         !  mbase>=0;  N.B. qs check done later with qbass
     &            max(b_wetfac(tile)%data(iq)*qs(iq,1),b_qg(tile)%data(iq,k980),b_qg(tile)%data(iq,k))/b_qg(tile)%data(iq,k)
         enddo 
        endif  ! (mbase==0)
      ...
      if(mbase==1)then ! fg; qg1   similar to -10  
         do iq=1,imax
          ...
          k=kkbb(iq)  ! for alfqarr calc
c         N.B. if fg<=0, then alfqarr will keep its input value, e.g. 1.25         
         if(b_fg(tile)%data(iq)>0.)alfqarr(iq)=alfqarr(iq)*         !  N.B. qs check done later with qbass
     &               max(b_qg(tile)%data(iq,1),b_qg(tile)%data(iq,k980),b_qg(tile)%data(iq,k))/b_qg(tile)%data(iq,k)
         enddo 
        endif  ! (mbase==1)
      ...
      if(mbase==2)then ! fg; qg1    fixed-up mbase=0
         do iq=1,imax
          ...
          qs(iq,1)=.622*es(iq,1)/max(pk-es(iq,1),0.1)  
          k=kkbb(iq)  ! for alfqarr calc
          if(b_fg(tile)%data(iq)>0.)alfqarr(iq)=max(b_wetfac(tile)%data(iq)*qs(iq,1),  !  N.B. qs check done later with qbass
     &           alfqarr(iq)*b_qg(tile)%data(iq,k980),alfqarr(iq)*b_qg(tile)%data(iq,k))/b_qg(tile)%data(iq,k)
         enddo 
        endif  ! (mbase==2)
      ...
      if(mbase==3)then ! fg; qg1   combined 1 & 2
         do iq=1,imax
          ...
          qs(iq,1)=.622*es(iq,1)/max(pk-es(iq,1),0.1)  
          k=kkbb(iq)  ! for alfqarr calc
          if(b_land(tile)%data(iq))then
            if(b_fg(tile)%data(iq)>0.)alfqarr(iq)=alfqarr(iq)*         !  same as mbase=1
     &            max(b_qg(tile)%data(iq,1),b_qg(tile)%data(iq,k980),b_qg(tile)%data(iq,k))/b_qg(tile)%data(iq,k)
          else
            if(b_fg(tile)%data(iq)>0.)alfqarr(iq)=max(qs(iq,1),        !  same as mbase=2
     &           alfqarr(iq)*b_qg(tile)%data(iq,k980),alfqarr(iq)*b_qg(tile)%data(iq,k))/b_qg(tile)%data(iq,k)
          endif
         enddo 
        endif  ! (mbase==3)
      ...
      if(mbase==4)then ! fg; qg_k   similar to combined 1 & 2 with qs limit - now preferred
         do iq=1,imax
          ...
          if(b_land(tile)%data(iq))then
            if(b_fg(tile)%data(iq)>0.)alfqarr(iq)=min(qs(iq,k),alfqarr(iq)*       
     &               max(b_qg(tile)%data(iq,1),b_qg(tile)%data(iq,k980),b_qg(tile)%data(iq,k)))
     &              /max(b_qg(tile)%data(iq,k),qgmin)
          else
            if(b_fg(tile)%data(iq)>0.)alfqarr(iq)=max(1.,qs(iq,k)
     &              /max(b_qg(tile)%data(iq,k),qgmin))
          endif
         enddo 
      endif  ! (mbase==4)

      if(nproc==1.and.serial)write(6,*) 'max_alfqarr,alfin:',
     &                    maxval(alfqarr),maxval(alfin)

      if(ktau==1.and.mydiag.and.have_idjd)
     &   write(6,"('alfqarr',2f7.3)") alfqarr(idjd)

!     just does convective; L/S rainfall done later by LDR scheme
      qliqw(:,:)=0.  ! before itn
      kmin(:)=0
      conrev(1:imax)=1000.*b_ps(tile)%data(:)/(grav*dt) ! factor to convert precip to g/m2/s
      rnrt(:)=0.       ! initialize large-scale rainfall array; before itn
      rnrtc(:)=0.      ! initialize convective  rainfall array; before itn
      kbsav_ls(:)=0    ! for L/S
!!      b_ktsav(tile)%data(:)=kl-1    ! preset value to show no deep or shallow convection
!!      b_kbsav(tile)%data(:)=kl-1    ! preset value to show no deep or shallow convection

      nuv=0
      fluxt(:,:)=0.     ! just for some compilers
      b_fluxtot(tile)%data(:,:)=0.  ! diag for MJT  - total mass flux at level k-.5
...
!       calculate hs
      ...
      if(itn==1)then  !--------------------------------------
        ...
        if(nbase<=-1.and.nbase>=-3)then  ! i.e. -1, -2 or -3
         do k=1,kl-1
          do iq=1,imax
           if(k>kkbb(iq))qbass(iq,k)=min(b_alfin(tile)%data(iq)*qq(iq,k),qbass(iq,k))  ! above PBL; qbass only defined for itn=1
          enddo
         enddo  ! k loop
...
c***    Also entrain may slow convergence   N.B. qbass only used in next few lines
     ...
     .           ntest,methdetr,methprec,detrain
      ...
      if((ntest>0.or.nmaxpr==1).and.mydiag.and.have_idjd) then
        iq=idjd
     ...
     .             (1000.*b_qlg(tile)%data(iq,k),k=1,kl)
        if(itn==1)write (6,"('qfg  ',12f7.3/(5x,12f7.3))") 
     .             (1000.*b_qfg(tile)%data(iq,k),k=1,kl)
        if(itn==1)write (6,"('qtot ',12f7.3/(5x,12f7.3))")
     &        (1000.*(qq(iq,k)+b_qlg(tile)%data(iq,k)+b_qfg(tile)%data(iq,k)),k=1,kl)
        if(itn==1)write (6,"('qtotx',12f7.3/(5x,12f7.3))")
     &        (10000.*dsk(k)*(qq(iq,k)+b_qlg(tile)%data(iq,k)+b_qfg(tile)%data(iq,k)),k=1,kl)
        pwater0=0.   ! in mm     
        do k=1,kl
         iq=idjd
         ...
         q0(k)=qq(iq,k)
         t0(k)=tt(iq,k)
         pwater0=pwater0-dsig(k)*b_qg(tile)%data(iq,k)*b_ps(tile)%data(iq)/grav
        enddo
!       following prints are just preliminary values. Others further down  
     ...
     .             s(iq,:)/cp+hlcp*qplume(iq,:)
        ...
        write (6,"('cfracc',12f7.3/6x,12f7.3)") b_cfrac(tile)%data(iq,:)
...
!        find tentative cloud base, and bottom of below-cloud layer
     ...
     &       .and.qplume(iq,k-1)>max(qs(iq,k),qq(iq,k)))then   
             ...
             kb_sav(iq)=k-1
             kt_sav(iq)=k
             if(k>kkbb(iq)+1)alfqarr(iq)=b_alfin(tile)%data(iq)
         endif   ! (kdown(iq)>0...)
...
!          find tentative cloud base as peak of hplume if above PBL
     ...
     &  .and.b_dpsldt(tile)%data(iq,k)<0.)then     
             kb_sav(iq)=k
             kt_sav(iq)=k+1
             kdown(iq)=0
          endif  
...
!        N.B. bit noisy if allow cloud base too much above kkbb
     ...
     &             itn,iq,ka,kt_sav(iq)
      ...
      enddo
      if(nmaxpr==1.and.mydiag.and.have_idjd)write(6,*) 
     &       'kb_saved,kt_saved,timeconva',
     &       kb_saved(idjd), kt_saved(idjd),b_timeconv(tile)%data(idjd)
      ...
      do k=2,kl-2   ! upwards to find cloud top  
         do iq=1,imax
          if(k>kb_sav(iq).and.kdown(iq)==1)then 
            entrsav(iq,k)=-b_entrainn(tile)%data(iq)*dsig(k) ! (linear) entrained mass into plume_k
...
c      detxsav(iq,kt_sav(iq))=0.
      ...
      if(ntest>0.and.mydiag.and.have_idjd)then
...
!      and modify fluxv and entrsav  *** assumes entrain<0.  ***     
     ...
     &             fluxv(idjd,1:kt_sav(idjd))
      ...
      if(nmaxpr==1.and.mydiag.and.have_idjd)then
        iq=idjd
        write (6,"('qplume',f6.3,11f7.3/(5x,12f7.3))")1000.*qplume(iq,:)
        write (6,"('splume',12f7.2/(5x,12f7.2))")splume(iq,:)/cp 
...
!     apply entrainment and detrainment effects to environment    
     ...
     &       -detrainx,0.)   ))
        enddo
        if(mydiag.and.nmaxpr==1.and.have_idjd)
...
!      tdown is temperature emerging from downdraft (at cloud base)       
       ...
       tdown(iq)=b_t(tile)%data(iq,kb_sav(iq))+(s(iq,kdown(iq))-s(iq,kb_sav(iq))
...
!    .           (tdown(iq)-b_t(tile)%data(iq,kb_sav(iq)))*dqsdt(iq,kb_sav(iq)) )
...
!      typically fldown=.3 or -.3      
     ...
     &                abs(fldown)*(sig(kb_sav(iq))-sig(kt_sav(iq))))
       ...
       if(tdown(iq)>b_t(tile)%data(iq,kb_sav(iq)).or.totprec<0.
     &    .or.sig(kb_sav(iq))-sig(kt_sav(iq))<.4)then   ! suppr. downdraft
       ...
       if(ntest==1.and.iq==idjd.and.mydiag.and.have_idjd)then
...
!      add in downdraft contributions at level kdown
       ...
       if(nuv>0)then
         delu(iq,kt_sav(iq))=b_u(tile)%data(iq,kb_sav(iq))  ! no entrainment effects included for u,v
         delv(iq,kt_sav(iq))=b_v(tile)%data(iq,kb_sav(iq))   ! so assuming unit upward flux through depth of plume
         delu(iq,kdown(iq))=delu(iq,kdown(iq))-fldow(iq)*b_u(tile)%data(iq,kdown(iq))
         delv(iq,kdown(iq))=delv(iq,kdown(iq))-fldow(iq)*b_v(tile)%data(iq,kdown(iq))
       endif  ! (nuv>0)
      enddo  ! iq loop
...
!         calculate emergent downdraft properties
     ...
     &        (s(iq,kb_sav(iq))+cp*(tdown(iq)-b_t(tile)%data(iq,kb_sav(iq))))  ! correct
!         subtract contrib from cloud base layers into plume
          ...
          if(nuv>0)then
             delu(iq,k)=-upin(k,kb_sav(iq))*b_u(tile)%data(iq,kb_sav(iq))
     &                 +fldow(iq)*downex(k,kb_sav(iq))*b_u(tile)%data(iq,kdown(iq))
             delv(iq,k)=-upin(k,kb_sav(iq))*b_v(tile)%data(iq,kb_sav(iq))
     &                 +fldow(iq)*downex(k,kb_sav(iq))*b_v(tile)%data(iq,kdown(iq))
          endif  ! (nuv>0)
         enddo   ! k loop             ######
      ...
      do k=2,kl-1
       do iq=1,imax
         if(fluxv(iq,k-1)>0.)then  ! downwards
           ...
           if(nuv>0)then
             delu(iq,k-1)=delu(iq,k-1)+fluxv(iq,k-1)*b_u(tile)%data(iq,k)
             delv(iq,k-1)=delv(iq,k-1)+fluxv(iq,k-1)*b_v(tile)%data(iq,k)
             delu(iq,k)=delu(iq,k)-fluxv(iq,k-1)*b_u(tile)%data(iq,k)
             delv(iq,k)=delv(iq,k)-fluxv(iq,k-1)*b_v(tile)%data(iq,k)
           endif
           ...
           if(nuv>0)then
             delu(iq,k-1)=delu(iq,k-1)+fluxv(iq,k-1)*b_u(tile)%data(iq,k-1)
             delv(iq,k-1)=delv(iq,k-1)+fluxv(iq,k-1)*b_v(tile)%data(iq,k-1)
             delu(iq,k)=delu(iq,k)-fluxv(iq,k-1)*b_u(tile)%data(iq,k-1)
             delv(iq,k)=delv(iq,k)-fluxv(iq,k-1)*b_v(tile)%data(iq,k-1)
           endif
        endif
       enddo
      enddo
...
!     calculate actual delq and dels
      ...
      if(diag.and.mydiag.and.have_idjd)then   ! JLM
        iq=idjd
...
!          i.e. alfqarr*[qq+M*delq]_kb=[qs+M*dqsdt*dels/cp]_kb+1
     ...
     .     (dqsdt(iq,k+1)*dels(iq,k+1)/cp +alfqarr(iq)*abs(delq(iq,k))))
           ...
           b_convpsav(tile)%data(iq)=fluxq(iq)
...
!       pre 21/7/04 occasionally got -ve fluxt, for supersaturated layers
     ...
     &       fluxt(iq,k)<b_convpsav(tile)%data(iq))then  
!         if(fluxt(iq,k)<b_convpsav(tile)%data(iq))then  ! superseded by preceding line
            b_convpsav(tile)%data(iq)=fluxt(iq,k)
     ...
     &             b_convpsav(tile)%data(iq)=0. 
          if(dels(iq,kt_sav(iq))<=0.)b_convpsav(tile)%data(iq)=0.    ! JLM 1505 must stabilize
     ...
     &              k,splume(iq,k-1)+hl*qplume(iq,k-1)-hs(iq,k)     
      ...
      if(ntest==2.and.mydiag)then     !######################
        convmax=0.
        do iq=1,imax
         if(b_convpsav(tile)%data(iq)>convmax.and.kb_sav(iq)==2)then
           write(6,*) 'ktau,iq,convpsav,fluxt3,kb_sav2,kt_sav ',
     &            ktau,iq,b_convpsav(tile)%data(iq),fluxt(iq,3),kb_sav(iq),kt_sav(iq)
           convmax=b_convpsav(tile)%data(iq)
         endif
        enddo
      endif    ! (ntest==2.and.mydiag)    !######################
      if(ntest==2.and.mydiag.and.have_idjd)then     !######################
        iq=idjd
        k=kb_sav(iq)
        if(k<kl)then
     ...
     &    kmin(iq),fluxq(iq),b_convpsav(tile)%data(iq)      
        write(6,"('delQ*dsk6p',6p9f8.3/(10x,9f8.3))")
     .    (b_convpsav(tile)%data(iq)*delq(iq,k)*dsk(k),k=1,kl)
        write(6,"('delt*dsk3p',3p9f8.3/(10x,9f8.3))")
     .   (b_convpsav(tile)%data(iq)*dels(iq,k)*dsk(k)/cp,k=1,kl)
        convmax=0.
        nums=0
     ...
     &         '  rhb  rht   rnrt    rnd'
      ...
      if(ntest==2.and.mydiag)then     !######################
        do iq=1,imax     
         if(kt_sav(iq)-kb_sav(iq)==1.and.b_convpsav(tile)%data(iq)>0.)then
           nums=nums+1
           if(b_convpsav(tile)%data(iq)>convmax)then
             convmax=b_convpsav(tile)%data(iq)
             write(6,"('bc  ',3i5,2i3,2x,2f7.3,f6.3,2f5.2,2f7.4)") 
     .       ktau,iq,nums,kb_sav(iq),kt_sav(iq),
     .       dsk(kb_sav(iq)),
     .       fluxt(iq,kt_sav(iq)),b_convpsav(tile)%data(iq),
     .       qq(iq,kb_sav(iq))/qs(iq,kb_sav(iq)),
     .       qq(iq,kt_sav(iq))/qs(iq,kt_sav(iq)),
     &       rnrtcn(iq),rnrtcn(iq)*b_convpsav(tile)%data(iq)*b_ps(tile)%data(iq)/grav
           endif
         endif
        enddo  ! iq loop
        convmax=0.
        nums=0
        do iq=1,imax     
         if(kt_sav(iq)-kb_sav(iq)==2.and.b_convpsav(tile)%data(iq)>0.)then
           nums=nums+1
           if(b_convpsav(tile)%data(iq)>convmax)then
             convmax=b_convpsav(tile)%data(iq)
     ...
     &       rnrtcn(iq),rnrtcn(iq)*b_convpsav(tile)%data(iq)*b_ps(tile)%data(iq)/grav
        ...
        nums=0
        do iq=1,imax     
         if(kt_sav(iq)-kb_sav(iq)==3.and.b_convpsav(tile)%data(iq)>0.)then
           nums=nums+1
           if(b_convpsav(tile)%data(iq)>convmax)then
             convmax=b_convpsav(tile)%data(iq)
...
!        diff=0, .14, .2, .3, .4, .5, .6, .7 gives detrainn=1, 1., 1., .69, .35, .17, .11, .1     for .1  X
      ...
      if(itn==1)then   ! ************************************************
        ...
        do k=2,kl-1
         do iq=1,imax
          if(k>kb_sav(iq).and.k<=kt_sav(iq))then
            kb=kb_sav(iq)
            b_cape(tile)%data(iq)=b_cape(tile)%data(iq)-(splume(iq,kb)+hl*qplume(iq,kb)-hs(iq,k))*
     &              rdry*dsig(k)/(cp*sig(k))            
       ...
       if(convtime>100.)then  ! land/sea effects, with tied_con irrelevant
           do iq=1,imax    
             if(b_land(tile)%data(iq))then  ! e.g. for 3060.60 
...
!        N.B. tied_con now only used convtime < -100    
         if(tied_con>2.)then   ! now omega_900<0 or fg>tied_con test                                         
           do iq=1,imax    
             if(b_fg(tile)%data(iq)>tied_con.or.b_dpsldt(tile)%data(iq,k900)<0.)then  ! e.g. for -2099.60 t_c=40
               convtim_deep(iq)=mcontlnd
             else
               convtim_deep(iq)=mcontsea
             endif
         ...
         elseif(tied_con>1.)then   !  e.g. 1.85       omega.or.pbl test
           do iq=1,imax    
             if(sig(kb_sav(iq))<tied_con-1..or.b_dpsldt(tile)%data(iq,k900)<0.)then ! e.g. -2040.60 t_c=1.85
               convtim_deep(iq)=mcontlnd
             else
               convtim_deep(iq)=mcontsea
             endif
         ...
         elseif(tied_con<-.99)then ! for magnitude_of_omega test
           ...
           omgtst(:)=max(min(b_dpsldt(tile)%data(:,k900)/sumb,1.),0.) ! used for lin interp
           convtim_deep(:)=mcontsea+omgtst(:)*(mcontlnd-mcontsea)
         elseif(tied_con>0.)then   !  e.g. tied_con = .85;  omega.and.pbl test
           do iq=1,imax    
             if(b_dpsldt(tile)%data(iq,k900)<0.)then 
               convtim_deep(iq)=mcontlnd*
     ...
     &          max(1.,(1.-tied_con)/max(.01,1-sig(kb_sav(iq))))
         ...
         elseif(tied_con<0.)then                      !  e.g. .tied_con = -.9
           do iq=1,imax    
             if(sig(kb_sav(iq))<-tied_con.and.b_dpsldt(tile)%data(iq,k900)<0.)then ! e.g. -2040.60 t_c=-.9
               convtim_deep(iq)=mcontlnd
             else
               convtim_deep(iq)=mcontsea
             endif
...
c           print *,'has tied_con=0'
        ...
        if(nmaxpr==1.and.mydiag.and.have_idjd)then
         ...
         write(6,*) 'timeconvb,b',b_timeconv(tile)%data(idjd)
     &    ,max(0.,min(sig(kb_sav(idjd))-sig(k)-.2,.4)*mdelay/.4) 
        endif
        do iq=1,imax
         if(b_convpsav(tile)%data(iq)>0.)then ! used by mdelay options
           b_timeconv(tile)%data(iq)=b_timeconv(tile)%data(iq)+dt/60. ! now in minutes
         else       ! usual
           b_timeconv(tile)%data(iq)=0.
         endif
        enddo
...
!     &     b_fg(tile)%data(iq),sumb,convtim_deep(iq),sig(kb_sav(iq))-sig(kt_sav(iq)),
!     &    1.e8*b_dpsldt(tile)%data(iq,k900),factr(iq),kb_sav(iq),kt_sav(iq)
         enddo
        if(nmaxpr==1.and.mydiag.and.have_idjd)then
        iq=idjd
        write(6,*)'timeconvc,convpsav,sig_b,sig_t,convt_frac,factr_a',
     &      b_timeconv(tile)%data(iq),b_convpsav(tile)%data(iq),sig(kb_sav(iq)),sig(kt_sav(iq)),
     &      convt_frac,factr(iq)
        endif
...
!         tied_b=10 gives factr [1, .917, .786, .611,.3,.100] for ds = [200, 100, 50, 25,8,2} km     
          do iq=1,imax
            summ=ds/(b_em(tile)%data(iq)*208498.)
            factr(iq)=factr(iq)*(1.+tied_b)*summ/(1.+tied_b*summ)
          enddo
       endif  ! (tied_b>1.)
      do iq=1,imax
         b_kbsav(tile)%data(iq)=kb_sav(iq)   ! just value at end of itn=1  Apr'15
         b_ktsav(tile)%data(iq)=kt_sav(iq)  
      enddo   ! iq loop
      endif    ! (itn==1)     ! ************************************************

      if(nmaxpr==1.and.nevapcc.ne.0.and.mydiag.and.have_idjd)then
       write (6,
     ...
     &   b_timeconv(tile)%data(idjd),b_entrainn(tile)%data(idjd),factr(idjd) 
      endif

!      do iq=1,imax
!       if(b_ktsav(tile)%data(iq)==kl-1.and.b_convpsav(tile)%data(iq)>0.)then
!         b_kbsav(tile)%data(iq)=kb_sav(iq) 
!         b_ktsav(tile)%data(iq)=kt_sav(iq)  
!       endif  ! (b_ktsav(tile)%data(iq)==kl-1.and.b_convpsav(tile)%data(iq)>0.)
!      enddo   ! iq loop

      if(itn<iterconv.or.iterconv<0)then   ! iterconv<0 calls convjlm twice, and does convfact*
        b_convpsav(tile)%data(:)=convfact*b_convpsav(tile)%data(:) ! typically convfact=1.05
!       b_convpsav(tile)%data(:)=convfact*b_convpsav(tile)%data(:)*factr(:) ! dangerous tried on 7/3/14 
      endif                              ! (itn<iterconv.or.iterconv<0)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      rnrtcn(:)=rnrtcn(:)-qxcess(:)

!     update qq, tt and precip
      rnrtc(:)=rnrtc(:)+b_convpsav(tile)%data(:)*rnrtcn(:)*conrev(:) ! g/m**2/s
      if(ncvcloud==0)then  ! usual
        do k=1,kl   
         do iq=1,imax
          qq(iq,k)=qq(iq,k)+b_convpsav(tile)%data(iq)*delq(iq,k)
          tt(iq,k)=tt(iq,k)+b_convpsav(tile)%data(iq)*dels(iq,k)/cp
         enddo    ! iq loop
        enddo     ! k loop
...
!        fluxt here denotes delt, phi denotes del_phi
           do iq=1,imax
            fluxt(iq,1)=b_convpsav(tile)%data(iq)*dels(iq,1)/(cp+bet(1))
            phi(iq,1)=bet(1)*fluxt(iq,1)  ! phi correction done for ncvcloud.ne.0
            tt(iq,1)=tt(iq,1)+fluxt(iq,1)
           enddo    ! iq loop
          do k=2,kl
           do iq=1,imax
            fluxt(iq,k)=(b_convpsav(tile)%data(iq)*dels(iq,k)-phi(iq,k-1)
...
!     N.B. convpsav has been updated here with convfact, but without factr term  
      if(methdetr<0)then
       do k=2,kl-1 
        do iq=1,imax  
         qliqw(iq,k)=qliqw(iq,k)+b_convpsav(tile)%data(iq)*delqliqw(iq,k)    ! after applying convpsav, accumulating for itns
        enddo  ! iq loop
       enddo   ! k loop
      else
       do k=2,kl-1 
        do iq=1,imax  
         if(k>kb_sav(iq).and.k<=kt_sav(iq))then
           deltaq=b_convpsav(tile)%data(iq)*qxcess(iq)*
     &                   detrarr(k,kb_sav(iq),kt_sav(iq))/dsk(k)
      ...
      if(ntest>0.and.mydiag.and.have_idjd)then
        iq=idjd
        print *,'liqw',b_convpsav(tile)%data(iq),qxcess(iq),kb_sav(iq),kt_sav(iq),
...
!       N.B. convpsav(iq) is already mult by dt jlm: mass flux is convpsav/dt
     ...
     .             (tt(iq,k),k=1,kl)
        ...
        write(6,*) 'rnrtcn,convpsav ',rnrtcn(iq),b_convpsav(tile)%data(iq)
     ...
     .         qplume(iq,kb_sav(iq)),qs(iq,kt_sav(iq)),qq(iq,kt_sav(iq)) 
        ...
        heatlev=0.
        do k=1,kl
         delq_av=delq_av+dsk(k)*b_convpsav(tile)%data(iq)*delq(iq,k)
         delt_av=delt_av+dsk(k)*b_convpsav(tile)%data(iq)*dels(iq,k)/cp
         heatlev=heatlev+sig(k)*dsk(k)*b_convpsav(tile)%data(iq)*dels(iq,k)/cp
        enddo
...
!     update u & v using actual delu and delv (i.e. divided by dsk)
      if(nuvconv.ne.0.or.nuv>0)then
        if(ntest>0.and.mydiag.and.have_idjd)then
          ...
          write (6,"('u  ',12f7.2/(3x,12f7.2))") b_u(tile)%data(idjd,:)
          write (6,"('v  ',12f7.2/(3x,12f7.2))") b_v(tile)%data(idjd,:)
        endif  ! (ntest>0.and.mydiag)
        do k=1,kl-2   
         do iq=1,imax
          b_u(tile)%data(iq,k)=b_u(tile)%data(iq,k)+facuv*factr(iq)*b_convpsav(tile)%data(iq)*delu(iq,k)/dsk(k)
          b_v(tile)%data(iq,k)=b_v(tile)%data(iq,k)+facuv*factr(iq)*b_convpsav(tile)%data(iq)*delv(iq,k)/dsk(k)
         enddo  ! iq loop
        enddo   ! k loop
        if(ntest>0.and.mydiag.and.have_idjd)then
...
!     section for convective transport of trace gases (jlm 22/2/01)
      if(ngas>0)then
        do ntr=1,ngas
         do k=1,kl-2
          do iq=1,imax
           s(iq,k)=b_tr(tile)%data(iq,k,ntr)
          enddo    ! iq loop
         enddo     ! k loop
         do iq=1,imax
          if(kt_sav(iq)<kl-1)then
            kb=kb_sav(iq)
            kt=kt_sav(iq)
            veldt=factr(iq)*b_convpsav(tile)%data(iq)*(1.-fldow(iq)) ! simple treatment
            fluxup=veldt*s(iq,kb)
!           remove gas from cloud base layer
            b_tr(tile)%data(iq,kb,ntr)=b_tr(tile)%data(iq,kb,ntr)-fluxup/dsk(kb)
!           put flux of gas into top convective layer
            b_tr(tile)%data(iq,kt,ntr)=b_tr(tile)%data(iq,kt,ntr)+fluxup/dsk(kt)
            do k=kb+1,kt
             b_tr(tile)%data(iq,k,ntr)=b_tr(tile)%data(iq,k,ntr)-s(iq,k)*veldt/dsk(k)
             b_tr(tile)%data(iq,k-1,ntr)=b_tr(tile)%data(iq,k-1,ntr)+s(iq,k)*veldt/dsk(k-1)
            enddo
          endif
         enddo   ! iq loop
        enddo    ! ntr loop    
      ...
      if ( abs(iaero)>=2 ) then
        do ntr = 1,naero
          xtgscav(:,:) = 0.
          s(:,1:kl-2) = b_xtg(tile)%data(:,1:kl-2,ntr)
          do iq = 1,imax
           if ( kt_sav(iq)<kl-1 ) then
             kb = kb_sav(iq)
             kt = kt_sav(iq)
             ...
             do k = kb,kt
               ttsto(k) = b_t(tile)%data(iq,k) + factr(iq)*(tt(iq,k)-b_t(tile)%data(iq,k))
               qqsto(k) = b_qg(tile)%data(iq,k) + factr(iq)*(qq(iq,k)-b_qg(tile)%data(iq,k))
               qqold(k) = b_qg(tile)%data(iq,k) + factr(iq)*(qqsav(iq,k)-b_qg(tile)%data(iq,k))
               qlsto(k) = factr(iq)*qliqw(iq,k)
               qlold(k) = factr(iq)*qliqwsav(iq,k)
               rho(k) = b_ps(tile)%data(iq)*sig(k)/(rdry*ttsto(k))
               ...
               xtgtmp(k) = b_xtg(tile)%data(iq,k,3)
             end do
...
!            remove aerosol from lower layer
             b_xtg(tile)%data(iq,kb,ntr) = b_xtg(tile)%data(iq,kb,ntr)-fluxup/dsk(kb)
!            put flux of aerosol into upper layer
             b_xtg(tile)%data(iq,kt,ntr) = b_xtg(tile)%data(iq,kt,ntr)+fluxup*(1.-fscav(kt))
     &          /dsk(kt)
             xtgscav(iq,kt)=fluxup*fscav(kt)/dsk(kt)
             do k = kb+1,kt
              b_xtg(tile)%data(iq,k,ntr)=b_xtg(tile)%data(iq,k,ntr)-s(iq,k)*veldt/dsk(k)
              b_xtg(tile)%data(iq,k-1,ntr)=b_xtg(tile)%data(iq,k-1,ntr)+s(iq,k)*(1.-fscav(k-1))
     &           *veldt/dsk(k-1)
          ...
          if (ntr==itracso2) then
            do k=1,kl
              b_so2wd(tile)%data(:)=b_so2wd(tile)%data(:)+xtgscav(:,k)*b_ps(tile)%data(:)*dsk(k)
     &          /(grav*dt)
            end do
          elseif (ntr==itracso2+1) then
            do k=1,kl
              b_so4wd(tile)%data(:)=b_so4wd(tile)%data(:)+xtgscav(:,k)*b_ps(tile)%data(:)*dsk(k)
     &          /(grav*dt)
            end do
          elseif (ntr==itracbc.or.ntr==itracbc+1) then
            do k=1,kl
              b_bcwd(tile)%data(:)=b_bcwd(tile)%data(:)+xtgscav(:,k)*b_ps(tile)%data(:)*dsk(k)
     &          /(grav*dt)
            end do
          elseif (ntr==itracoc.or.ntr==itracoc+1) then
            do k=1,kl
              b_ocwd(tile)%data(:)=b_ocwd(tile)%data(:)+xtgscav(:,k)*b_ps(tile)%data(:)*dsk(k)
     &          /(grav*dt)
            end do
          elseif (ntr>=itracdu.and.ntr<itracdu+ndust) then
            do k=1,kl
              b_dustwd(tile)%data(:)=b_dustwd(tile)%data(:)+xtgscav(:,k)*b_ps(tile)%data(:)*dsk(k)
     &          /(grav*dt)
            end do
          end if
        end do     ! nt loop
     ...
     &   b_cfrac(tile)%data(iq,kb_sav(iq)+1),entrsav(iq,kdown(iq)),
     &   detxsav(iq,kdown(iq)),fluxv(iq,kdown(iq)),
     &   fluxv(iq,kt_sav(iq)-1),factr(iq),b_convpsav(tile)%data(iq),
     &   rnrtcn(iq),b_cape(tile)%data(iq)
       dtsol=.01*b_sgsave(tile)%data(iq)/(1.+.25*(b_u(tile)%data(iq,1)**2+b_v(tile)%data(iq,1)**2))   ! solar heating   ! sea
       write(6,"('pblh,fldow,tdown,qdown,fluxq3',
     &             f8.2,f5.2,2f7.2,3p2f8.3,1pf8.2)")
     &     b_pblh(tile)%data(iq),fldow(iq),tdown(iq),qdown(iq),fluxq(iq)
       write(6,"('ktau,kkbb,wetfac,dtsol,alfqarr,fg,omega8
     &,omgtst',i5,i3,3f6.3,f8.3,8pf9.3,1pf9.3)") ktau,kkbb(idjd),
     &      b_wetfac(tile)%data(idjd),dtsol,alfqarr(idjd),b_fg(tile)%data(idjd),
...
!          itns 2 & 3 only update saved values if convection occurred         
      ...
      do k=1,kl-1
         do iq=1,imax
c         if(fluxv(iq,k)>1.)b_fluxtot(tile)%data(iq,k)=b_fluxtot(tile)%data(iq,k)+ 
          b_fluxtot(tile)%data(iq,k)=b_fluxtot(tile)%data(iq,k)+   
     &          fluxv(iq,k)*factr(iq)*b_convpsav(tile)%data(iq)/dt  ! needs div by dt (check)
        enddo
      enddo
...
!------------------------------------------------ end of iterations ____#################################
      ...
      if(ntest>0.and.mydiag.and.have_idjd)
     &  write (6,"('C k,fluxtot6',i3,f7.2)")
     &                                  (k,1.e6*b_fluxtot(tile)%data(idjd,k),k=1,kl)
      if(nmaxpr==1.and.mydiag.and.have_idjd)then
     ...
     &          factr(idjd),kb_sav(idjd),kt_sav(idjd)
      ...
      do k=1,kl
      qq(1:imax,k)=b_qg(tile)%data(:,k)+factr(:)*(qq(1:imax,k)-b_qg(tile)%data(:,k))
      qliqw(1:imax,k)=factr(:)*qliqw(1:imax,k)      
      tt(1:imax,k)= b_t(tile)%data(:,k)+factr(:)*(tt(1:imax,k)- b_t(tile)%data(:,k))
      enddo
...
!           this is older simpler option, allowing ldr scheme to assign qfg without time complications          
            b_qlg(tile)%data(:,k)=b_qlg(tile)%data(:,k)+qliqw(1:imax,k)
          end do
        else
          do k=1,kl              
           do iq=1,imax
            if(tt(iq,k)<253.16)then   ! i.e. -20C
              b_qfg(tile)%data(iq,k)=b_qfg(tile)%data(iq,k)+qliqw(iq,k)
              tt(iq,k)=tt(iq,k)+3.35e5*qliqw(iq,k)/cp   ! fusion heating
            else
              b_qlg(tile)%data(iq,k)=b_qlg(tile)%data(iq,k)+qliqw(iq,k)
            endif
           enddo
          enddo  ! k loop           
        endif !  (rhmois==0.)
...
!_______________end of convective calculations__________________
     ...
     .   (qq(iq,k)+qliqw(iq,k)-b_qg(tile)%data(iq,k),k=1,kl)
        write(6,"('delT_t',12f7.3/(6x,12f7.3))")
     .   (tt(iq,k)-b_t(tile)%data(iq,k),k=1,kl)
      endif
      b_qg(tile)%data(:,:)=qq(1:imax,:)                   
      b_condc(tile)%data(:)=.001*dt*rnrtc(1:imax)      ! convective precip for this timestep
      b_precc(tile)%data(:)=b_precc(tile)%data(:)+b_condc(tile)%data(:)       
!    N.B. condx used to update rnd03,...,rnd24. LS added in leoncld.f       
      b_condx(tile)%data(:)=b_condc(tile)%data(:)+.001*dt*rnrt(1:imax) ! total precip for this timestep
      b_conds(tile)%data(:)=0.   ! (:) here for easy use in VCAM
      b_condg(tile)%data(:)=0.   ! (:) here for easy use in VCAM
      b_precip(tile)%data(:)=b_precip(tile)%data(:)+b_condx(tile)%data(:)
      b_t(tile)%data(:,:)=tt(1:imax,:)             

      if(ntest>0.or.(ktau<=2.and.nmaxpr==1).and.have_idjd)then
       if(mydiag)then
        iq=idjd
     ...
     .           fluxq(iq),b_convpsav(tile)%data(iq)
        write(6,"('fluxt',9f8.5/(5x,9f8.5))")
     .            (fluxt(iq,k),k=1,kt_sav(iq))
        pwater=0.   ! in mm     
        do k=1,kl
         pwater=pwater-dsig(k)*b_qg(tile)%data(iq,k)*ps(iq)/grav
        enddo
        write(6,*) 'pwater0,pwater+condx,pwater ',
     .           pwater0,pwater+b_condx(tile)%data(iq),pwater
        write(6,*) 'D condx ',b_condx(tile)%data(iq)
        write(6,*) 'precc,precip ',
     .           b_precc(tile)%data(iq),b_precip(tile)%data(iq)
       endif   ! (mydiag) needed here for maxmin
       call maxmin(rnrtc,'rc',ktau,1.,1)
      endif
      
      if(ntest==-1.and.nproc==1.and.ktau==10.and.serial)then
        do k=1,kl
