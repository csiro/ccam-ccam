...
!     could reinstate nbase=0 & kbsav_b from .f0406
      ...
      integer iaero
      real, dimension(:), allocatable, save :: alfqarr,alfqarrx,timeconv
...
c        alfqarrx reduces for finer resolution than 200 km grid            
        ...
        enddo
        if(mydiag)write(6,"('alfqarr,alfqarrx',2f7.3)") 
...
!     Following procedure chooses lowest valid cloud bottom (and base)
      ...
      do k=ka,kb,kc   ! downwards (for nbase=-4) to find cloud base **k loop**
...
!        find tentative cloud base ! 
!           (middle of k-1 level, uppermost level below pblh)
c        if(k>2.and.phi(iq,k-1)<pblh(iq)*grav)then
         if(k-1==kkbb(iq))then
           kb_sav(iq)=k-1
           kt_sav(iq)=k
           if(splume(iq,k-1)+hl*qplume(iq,k-1)<hs(iq,k).or.
     &        qplume(iq,k-1)<max(qs(iq,k),qq(iq,k)))then 
        ...
        enddo    ! iq loop
       else   !  nbase=-4 (searching downwards)
        do iq=1,ifull
...
c       kdown(:)=abs(kdown(:))
     ...
     &           (splume(iq,kb)+hl*qplume(iq,kb))/cp,hs(iq,k)/cp
      ...
      if(nevapcc>0)then ! typically 20, 30, 40, 60  ! uses **2
        do iq=1,ifull
           entrainn(iq)=.1*nevapcc*min(1.,                                          
     &         (.1/(max(.001,sig(kb_saved(iq)) -sig(kt_saved(iq)))))**2)
c            for dsig (.1,.2,.3,.4,.5,.6) 2nd term is (1, .25, .11, .06,  .04,  .03)   
      ...
      if(nevapcc<0)then ! typically -20, -30, -40, -60
        do iq=1,ifull
           entrainn(iq)=-.1*nevapcc*min(1.,                                          
     &         .1/(max(.001,sig(kb_saved(iq)) -sig(kt_saved(iq)))))
...
!     - usually with methprec=7 or 8 too
     ...
     &           1.42-2.1*(sig(kb_sav(iq))-sig(kt_sav(iq)))))
      ...
      if(methdetr==3)then  ! now usual
        do iq=1,ifull     
         detrain=min( 1., detrainin+(1.-detrainin)*(
     &      (.6-min(sig(kb_sav(iq))-sig(kt_sav(iq)), .6))/(.6-.15))**3 )
c        diff=0, .15, .2,.3, .4, .6 gives detrain=1, 1, .75, .406,.225, .15   for .15  - usual
c        diff=0, .15, .2,.3, .4, .6 gives detrain=1, 1, .732, .367,.179, .1   for .1  
         qxcess(iq)=detrain*rnrtcn(iq) 
        enddo
      endif   ! (methdetr==3)     
      if(methdetr>4)then  
...
!    in cludes methprec=7 & 8 but using detrarr array
c      detrfactr(:)=0.
       do k=kuocb+1,kl-1 
        do iq=1,ifull  
         if(k>kb_sav(iq).and.k<=kt_sav(iq))then
...
!     here the rainfall rate rnrt has been converted to g/m**2/sec
     ...
     .             (tt(iq,k),k=1,kl)
      ...
      if(nevapls.ne.0)then ! even newer UKMO (just for ldr=0)
        rKa=2.4e-2
        Dva=2.21
...
!__________________end of large-scale calculations (for ldr=0: rare setting)________
