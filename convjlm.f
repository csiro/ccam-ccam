...
!------------------------------------------------------------------------------
      ...
      real, dimension(:), allocatable, target, save ::  entrainn
      real, dimension(:), allocatable, target, save ::  timeconv
      real, dimension(:), allocatable, target, save ::  alfin
...
!           tied_over=10 gives factor [1, .917, .786, .611, .306] for ds = [200, 100, 50, 25, 8} km     
      ...
      real, dimension(:), pointer, contiguous :: lentrainn
...
c         N.B. if fg<=0, then alfqarr will keep its input value, e.g. 1.25         
     ...
     &                    maxval(alfqarr),maxval(alfin)
      ...
      if(ktau==1.and.mydiag.and.ntiles==1)
...
!     apply entrainment and detrainment effects to environment    
     ...
     &       -detrainx,0.)   ))
        enddo
        if(mydiag.and.nmaxpr==1.and.ntiles==1)
...
!      typically fldown=.3 or -.3      
     ...
     &    .or.sig(kb_sav(iq))-sig(kt_sav(iq))<.4)then   ! suppr. downdraft
       ...
       if(ntest==1.and.iq==idjd.and.mydiag.and.ntiles==1)then
...
!         tied_b=10 gives factr [1, .917, .786, .611,.3,.100] for ds = [200, 100, 50, 25,8,2} km     
      ...
      if(nmaxpr==1.and.nevapcc.ne.0.and.mydiag.and.ntiles==1)then
       write (6,
...
!------------------------------------------------ end of iterations ____#################################
      ...
      if(ntest>0.and.mydiag.and.ntiles==1)
...
!    N.B. condx used to update rnd03,...,rnd24. LS added in leoncld.f       
      ...
      if(ntest>0.or.(ktau<=2.and.nmaxpr==1).and.ntiles==1)then
       if(mydiag)then
        iq=idjd
