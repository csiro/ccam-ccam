...
real(kind=8), dimension(:), allocatable :: prfact,subdia,work ! use allocatable for cray compiler
...
shift = a(kl,kl-1)
if(abs(a(kl,kl))>1.e-99_8)shift = 0.0_8
if(abs(a(kl-1,kl))>1.e-99_8)shift = 0.0_8
if(abs(a(kl-1,kl-1))>1.e-99_8)shift = 0.0_8
m = kl
ns= 0
maxst = kl*10
...
do i=2,kl
  do k=i,kl
    if(abs(a(i-1,k))>1.e-99_8) fflag=.false.
  end do
end do
...
! decomposition
      ...
      r = abs(x)+abs(y)
      if(abs(r)<1.e-99_8)shift = a(m,m-1)
    end do
    z = a(k+2,k+1)*a(k+1,k)
    shift = 0.0_8
    ns = ns+1
...
if(ivec/=m)then
  ...
  r = 0.0_8
  do i=k,m
    if(abs(evalue-evr(i))<1.e-99_8.and.abs(evi(i))<1.e-99_8) then
      r = r+3.0_8
    end if
  end do
  evalue = evalue+r*ex
...
do i=1,k
  l = i+1
  iwork(i) = 0
  if (abs(a(i+1,i))<1.e-99_8.and.abs(a(i,i))<1.e-99_8) then
    a(i,i)=eps
    cycle
  else if (abs(a(i+1,i))<1.e-99_8) then
    cycle
  end if
...
do while (ncount<kl.and.iter<=2*kl)
  ...
  do i=1,kl
    ...
    if(column<1.e-99_8.or.row<1.e-99_8) then
      ncount = ncount + 1
      cycle
    end if
    q = column/row
