...
subroutine aldrloadoxidant(index,aa,tile,imax)
use aerodata_m, only : b_zoxidant
...
b_zoxidant(tile)%data(:,index)=aa(1:imax)
...
use aerodata_m, only : b_xtg,b_duste,b_dustdd,b_xtosav,b_xtg_solub,b_dmsso2o,b_so2so4o,b_dust_burden, &
                       b_bc_burden,b_oc_burden,b_dms_burden,b_so2_burden,b_so4_burden,b_EMISSFIELD
...
b_xtg(tile)%data(:,:,:) = max( b_xtg(tile)%data(:,:,:)+xte(:,:,:)*dt, 0. )
...
oldduste(:) = b_duste(tile)%data(:) ! duste is cumulative dust emissions
dcol1(:) = 0.
do nt = itracdu,itracdu+ndust-1
  dcol1(:) = dcol1(:) + sum( rhoa(:,:)*b_xtg(tile)%data(:,:,nt)*dz(:,:), dim=2 )
end do
! Calculate the settling of large dust particles
call dsettling(dt,rhoa,ttg,dz,aphp1(:,1:kl),tile,imax)
! Calculate dust emission and turbulent dry deposition at the surface
call dustem(dt,rhoa(:,1),wg,veff,dz(:,1),vt,snowd,tile,imax)
! Calculate integrated column dust after settling
dcol2(:) = 0.
do nt=itracdu,itracdu+ndust-1
  dcol2(:) = dcol2(:) + sum( rhoa(:,:)*b_xtg(tile)%data(:,:,nt)*dz(:,:), dim=2 )
end do
! Calculate deposition flux to surface
b_dustdd(tile)%data(:) = b_dustdd(tile)%data(:) + (dcol1(:)-dcol2(:))/dt + b_duste(tile)%data(:) - oldduste(:)

! Decay of hydrophobic black and organic carbon into hydrophilic forms
call xtsink(dt,xte,tile,imax)
b_xtg(tile)%data(:,:,:) = max( b_xtg(tile)%data(:,:,:)+xte(:,:,:)*dt, 0. )

! Compute diagnostic sea salt aerosol
call seasalt(land,fracice,zz,pblh,veff,tile,imax)

! Aerosol chemistry and wet deposition
! Need to invert vertical levels for ECHAM code... Don't you hate that?
do nt = 1,naero
  do k = 1,kl
    xtm1(:,kl+1-k,nt) = b_xtg(tile)%data(:,k,nt)
    ! Convert from aerosol concentration outside convective cloud (used by CCAM)
    ! to aerosol concentration inside convective cloud
    xtu(:,kl+1-k,nt) = max(b_xtg(tile)%data(:,k,nt)-(1.-clcon(:,k))*b_xtosav(tile)%data(:,k,nt),0.)/max(clcon(:,k),1.E-8)
  end do
end do
if ( aeromode>= 1 ) then
  do nt = 1,naero
    do k = 1,kl
      xliquid(:,kl+1-k,nt) = b_xtg_solub(tile)%data(:,k,nt)
    end do
  end do
else
  xliquid(:,:,:) = 0.    
...
do nt = 1,naero
  do k = 1,kl
    b_xtg(tile)%data(:,k,nt) = max( b_xtg(tile)%data(:,k,nt)+xte(:,kl+1-k,nt)*dt, 0. )
  end do
enddo
if ( aeromode>=1 ) then
  do nt = 1,naero
    do k = 1,kl
      b_xtg_solub(tile)%data(:,k,nt) = xliquid(:,kl+1-k,nt)  
    end do
  end do
end if
b_dmsso2o(tile)%data(:) = b_dmsso2o(tile)%data(:) + dmsoh(:) + dmsn3(:)             ! oxidation of DMS to SO2
b_so2so4o(tile)%data(:) = b_so2so4o(tile)%data(:) + so2oh(:) + so2h2(:) + so2o3(:)  ! oxidation of SO2 to SO4

burden(:) = 0.
do nt = 1,ndust
  burden(:) = burden(:) + sum( b_xtg(tile)%data(:,:,itracdu+nt-1)*rhoa(:,:)*dz(:,:), dim=2 )
end do
b_dust_burden(tile)%data(:) = b_dust_burden(tile)%data(:) + burden(:)

burden(:) = 0.
do nt = itracbc,itracbc+1
  burden(:) = burden(:) + sum( b_xtg(tile)%data(:,:,nt)*rhoa(:,:)*dz(:,:), dim=2 )
end do
b_bc_burden(tile)%data(:) = b_bc_burden(tile)%data(:) + burden(:)

burden(:) = 0.
do nt = itracoc,itracoc+1
  burden(:) = burden(:) + sum( b_xtg(tile)%data(:,:,nt)*rhoa(:,:)*dz(:,:), dim=2 )
end do
b_oc_burden(tile)%data(:) = b_oc_burden(tile)%data(:) + burden(:)

burden(:) = sum( b_xtg(tile)%data(:,:,itracdms)*rhoa(:,:)*dz(:,:), dim=2 )
b_dms_burden(tile)%data(:) = b_dms_burden(tile)%data(:) + burden(:)

burden(:) = sum( b_xtg(tile)%data(:,:,itracso2)*rhoa(:,:)*dz(:,:), dim=2 )
b_so2_burden(tile)%data(:) = b_so2_burden(tile)%data(:) + burden(:)

burden(:) = sum( b_xtg(tile)%data(:,:,itracso4)*rhoa(:,:)*dz(:,:), dim=2 )
b_so4_burden(tile)%data(:) = b_so4_burden(tile)%data(:) + burden(:)
...
use aerodata_m, only : b_EMISSFIELD,b_vso2,b_dmse,b_so2e,b_so4e,b_bce,b_oce,b_xtg, &
                       b_so2dd,b_so4dd,b_bcdd,b_ocdd
!
...
ZDMSCON(:) = b_EMISSFIELD(tile)%data(:,idmso)*(1.-SEAICEM(:))**2
...
where ( loland(:) )
  !zdmsemiss(:) = b_emissfield(tile)%data(:,idmst) !kg/m2/s
  zdmsemiss(:) = (1./1.938)*b_emissfield(tile)%data(:,idmst) !kgS/m2/s
elsewhere
...
PXTEMS(:,ITRACSO2)  =(b_EMISSFIELD(tile)%data(:,iso2a1)+b_EMISSFIELD(tile)%data(:,iso2b1))*0.97
PXTEMS(:,ITRACSO4)  =(b_EMISSFIELD(tile)%data(:,iso2a1)+b_EMISSFIELD(tile)%data(:,iso2b1))*0.03
...
do jk=jk2,jk3-1
  ...
  XTE(:,JK,ITRACSO2)  =XTE(:,JK,ITRACSO2)  +0.97*b_EMISSFIELD(tile)%data(:,iso2a2)*gdp !100% of the "above 100m" SO2 emission
  XTE(:,JK,ITRACSO4)  =XTE(:,JK,ITRACSO4)  +0.03*b_EMISSFIELD(tile)%data(:,iso2a2)*gdp !100% of the "above 100m" SO4 emission
end do
do jk=jk3,jk4-1
  gdp=1./(rhoa(:,jk)*dz(:,jk))/real(jk4-jk3)
  xte(:,jk,ITRACSO2)=xte(:,jk,ITRACSO2)+0.3*b_emissfield(tile)%data(:,iso2b2)*gdp
end do
do jk=jk4,jk5-1
  gdp=1./(rhoa(:,jk)*dz(:,jk))/real(jk5-jk4)
  xte(:,jk,ITRACSO2)=xte(:,jk,ITRACSO2)+0.4*b_emissfield(tile)%data(:,iso2b2)*gdp
end do
do jk=jk5,jk6-1
...
jk=1
gdp=1./(rhoa(:,jk)*dz(:,jk))
XTE(:,jk,ITRACSO2)=XTE(:,jk,ITRACSO2)+ZVOLCEMI*0.36*b_vso2(tile)%data(:)*gdp
do jk=jk4,jk6-1
...
do jk=jk8,jk9-1
  ...
  XTE(:,jk,ITRACSO2)=XTE(:,jk,ITRACSO2)+ZVOLCEMI*0.28*b_vso2(tile)%data(:)*gdp
end do
...
PXTEMS(:,ITRACBC)  =0.8*b_EMISSFIELD(tile)%data(:,ibca1)
PXTEMS(:,ITRACBC+1)=0.2*b_EMISSFIELD(tile)%data(:,ibca1)
PXTEMS(:,ITRACOC)  =0.5*(b_EMISSFIELD(tile)%data(:,ioca1)+b_EMISSFIELD(tile)%data(:,iocna))
PXTEMS(:,ITRACOC+1)=0.5*(b_EMISSFIELD(tile)%data(:,ioca1)+b_EMISSFIELD(tile)%data(:,iocna))
! Apply these here as a tendency (XTE), rather than as a surface flux (PXTEMS) via vertmix.
jk=1
gdp=1./(rhoa(:,jk)*dz(:,jk))
...
! Assume BC 80% hydrophobic, OC 50%.
PXTEMS(:,ITRACBC)  =0.8*b_EMISSFIELD(tile)%data(:,ibca2)
PXTEMS(:,ITRACBC+1)=0.2*b_EMISSFIELD(tile)%data(:,ibca2)
PXTEMS(:,ITRACOC)  =0.5*b_EMISSFIELD(tile)%data(:,ioca2)
PXTEMS(:,ITRACOC+1)=0.5*b_EMISSFIELD(tile)%data(:,ioca2)
! Apply these here as a tendency (XTE), rather than as a surface flux (PXTEMS) via vertmix.
do jk=jk2,jk3-1
  gdp=1./(rhoa(:,jk)*dz(:,jk))/real(jk3-jk2)
  xte(:,jk,itracbc)  =xte(:,jk,itracbc)  +pxtems(:,itracbc)*gdp
...
PXTEMS(:,ITRACBC)  =0.5*b_EMISSFIELD(tile)%data(:,ibcb1)
PXTEMS(:,ITRACBC+1)=0.5*b_EMISSFIELD(tile)%data(:,ibcb1)
PXTEMS(:,ITRACOC)  =0.5*b_EMISSFIELD(tile)%data(:,iocb1)
PXTEMS(:,ITRACOC+1)=0.5*b_EMISSFIELD(tile)%data(:,iocb1)
...
! Assume BC and OC both 50% hydrophobic.
PXTEMS(:,ITRACBC)  =0.5*b_EMISSFIELD(tile)%data(:,ibcb2)
PXTEMS(:,ITRACBC+1)=0.5*b_EMISSFIELD(tile)%data(:,ibcb2)
PXTEMS(:,ITRACOC)  =0.5*b_EMISSFIELD(tile)%data(:,iocb2)
PXTEMS(:,ITRACOC+1)=0.5*b_EMISSFIELD(tile)%data(:,iocb2)
! Apply these here as a tendency (XTE)
do jk=jk3,jk4-1
  gdp=1./(rhoa(:,jk)*dz(:,jk))/real(jk4-jk3)
...
do jk=1,kl
  b_dmse(tile)%data(:)=b_dmse(tile)%data(:)+xte(:,jk,ITRACDMS)*rhoa(:,jk)*dz(:,jk)   !Above surface
  b_so2e(tile)%data(:)=b_so2e(tile)%data(:)+xte(:,jk,ITRACSO2)*rhoa(:,jk)*dz(:,jk)   !Above surface
  b_so4e(tile)%data(:)=b_so4e(tile)%data(:)+xte(:,jk,ITRACSO4)*rhoa(:,jk)*dz(:,jk)   !Above surface
enddo

! Assume that BC and OC emissions are passed in through xte()
do jt=ITRACBC,ITRACBC+1
  do jk=1,kl
    b_bce(tile)%data(:)=b_bce(tile)%data(:)+xte(:,jk,jt)*rhoa(:,jk)*dz(:,jk)
  enddo
enddo
do jt=ITRACOC,ITRACOC+1
  do jk=1,kl
    b_oce(tile)%data(:)=b_oce(tile)%data(:)+xte(:,jk,jt)*rhoa(:,jk)*dz(:,jk)
  enddo
enddo

! Total biomass burning primary emissions (note 1.3 for organic carbon)
bbem=b_emissfield(tile)%data(:,ibcb1)+b_emissfield(tile)%data(:,ibcb2)+1.3*(b_emissfield(tile)%data(:,iocb1)+b_emissfield(tile)%data(:,iocb2))
...
zhilso2=b_xtg(tile)%data(:,1,itracso2)*(1.-exp(-ztmst*zvdrd(:,1)/dz(:,1)))/(ztmst*gdp)
xte(:,1,ITRACSO2)  =xte(:,1,ITRACSO2)  -zhilso2*gdp
  
zhilso4=b_xtg(tile)%data(:,1,itracso4)*(1.-exp(-ztmst*zvdrd(:,2)/dz(:,1)))/(ztmst*gdp)
xte(:,1,ITRACSO4)  =xte(:,1,ITRACSO4)  -zhilso4*gdp

ZHILBCO=b_xtg(tile)%data(:,1,ITRACBC)*(1.-exp(-ztmst*ZVDPHOBIC/dz(:,1)))/(ztmst*gdp)
xte(:,1,itracbc)  =xte(:,1,itracbc)    -zhilbco*gdp

ZHILBCY=b_xtg(tile)%data(:,1,ITRACBC+1)*(1.-exp(-ztmst*ZVDRD(:,2)/dz(:,1)))/(ztmst*gdp)
xte(:,1,itracbc+1)=xte(:,1,itracbc+1)  -zhilbcy*gdp

ZHILOCO=b_xtg(tile)%data(:,1,ITRACOC)*(1.-exp(-ztmst*ZVDPHOBIC/dz(:,1)))/(ztmst*gdp)
xte(:,1,itracoc)  =xte(:,1,itracoc)    -zhiloco*gdp

ZHILOCY=b_xtg(tile)%data(:,1,ITRACOC+1)*(1.-exp(-ztmst*ZVDRD(:,2)/dz(:,1)))/(ztmst*gdp)
xte(:,1,itracoc+1)=xte(:,1,itracoc+1)  -zhilocy*gdp

b_so2dd(tile)%data(:)=b_so2dd(tile)%data(:)+zhilso2
b_so4dd(tile)%data(:)=b_so4dd(tile)%data(:)+zhilso4
b_bcdd(tile)%data(:)=b_bcdd(tile)%data(:)+zhilbco+zhilbcy
b_ocdd(tile)%data(:)=b_ocdd(tile)%data(:)+zhiloco+zhilocy
...
SUBROUTINE XTSINK (PTMST,PXTE,tile,imax)
use aerodata_m, only : b_xtg
!
...
DO JK=1,kl
  ZXTP1=b_xtg(tile)%data(:,JK,ITRACBC)+PXTE(1:imax,JK,ITRACBC)*PTMST
  ZXTP1=ZXTP1*ZDECAY
  ZDXTDT=(ZXTP1-b_xtg(tile)%data(:,JK,ITRACBC))*PQTMST-PXTE(1:imax,JK,ITRACBC)
...
ZDECAY=EXP(ZFAC/86400.) ! 1 day
DO JK=1,kl
  ZXTP1=b_xtg(tile)%data(:,JK,ITRACOC)+PXTE(1:imax,JK,ITRACOC)*PTMST
  ZXTP1=ZXTP1*ZDECAY
  ZDXTDT=(ZXTP1-b_xtg(tile)%data(:,JK,ITRACOC))*PQTMST-PXTE(1:imax,JK,ITRACOC)
...
use aerodata_m, only : b_zoxidant,b_so2wd,b_so4wd,b_bcwd,b_ocwd,b_dustwd
...
ZZOH(:,:)   = b_ZOXIDANT(tile)%data(:,1:kl)
ZZH2O2(:,:) = b_ZOXIDANT(tile)%data(:,kl+1:2*kl)*PRHOP1(:,1:kl)*1.e-3
ZZO3(:,:)   = b_ZOXIDANT(tile)%data(:,2*kl+1:3*kl)*PRHOP1(:,1:kl)*1.e-3
ZZNO2(:,:)  = b_ZOXIDANT(tile)%data(:,3*kl+1:4*kl)*PRHOP1(:,1:kl)*1.e-3
...
!   CALCULATE NEW CHEMISTRY AND SCAVENGING TENDENCIES
  ...
  if ( jt==itracso2 ) then
    b_so2wd(tile)%data(:) = b_so2wd(tile)%data(:) + sum( zdep3d(:,:)*rhodz(:,:)*pqtmst, dim=2 )
  elseif ( jt==itracso4 ) then
    b_so4wd(tile)%data(:) = b_so4wd(tile)%data(:) + sum( zdep3d(:,:)*rhodz(:,:)*pqtmst, dim=2 )
  elseif ( jt==itracbc .or. jt==itracbc+1 ) then
    b_bcwd(tile)%data(:) = b_bcwd(tile)%data(:) + sum( zdep3d(:,:)*rhodz(:,:)*pqtmst, dim=2 )
  elseif ( jt==itracoc .or. jt==itracoc+1 ) then
    b_ocwd(tile)%data(:) = b_ocwd(tile)%data(:) + sum( zdep3d(:,:)*rhodz(:,:)*pqtmst, dim=2 )
  elseif ( jt>=itracdu .and. jt<itracdu+ndust ) then
    b_dustwd(tile)%data(:) = b_dustwd(tile)%data(:) + sum( zdep3d(:,:)*rhodz(:,:)*pqtmst, dim=2 )
  endif
...
real, dimension(:,:), intent(in) :: pfstayice
real, dimension(:,:), intent(in) :: pfstayliq
...
subroutine dsettling(tdt,rhoa,tmp,delz,prf,tile,imax)
use aerodata_m, only : b_xtg
...
do nt = 1, NDUST
  ...
  newxtg = b_xtg(tile)%data(:,kl,nt+itracdu-1)*exp(-b)
  newxtg = max( newxtg, 0. )
  dfall = max( b_xtg(tile)%data(:,kl,nt+itracdu-1) - newxtg, 0. )
  b_xtg(tile)%data(:,kl,nt+itracdu-1) = newxtg
  ...
  do k = kl-1,1,-1
    ...
    ! Fout = 1.-exp(-b)
    ! Fthru = 1.-Fout/b
    newxtg = b_xtg(tile)%data(:,k,nt+itracdu-1)*exp(-b) + dfall*(1.-exp(-b))/b
    newxtg = max( newxtg, 0. )
    dfall = max( b_xtg(tile)%data(:,k,nt+itracdu-1) + dfall - newxtg, 0. )
    b_xtg(tile)%data(:,k,nt+itracdu-1) = newxtg
  end do
...
subroutine dustem(tdt,rhoa,wg,w10m,dz1,vt,snowd,tile,imax)
use aerodata_m, only : b_erod,b_duste,b_xtg
...
do n = 1, ndust
  ...
  b_xtg(tile)%data(:,1,n+itracdu-1) = b_xtg(tile)%data(:,1,n+itracdu-1)*exp(-b*tdt) + a*tdt
  b_xtg(tile)%data(:,1,n+itracdu-1) = max( 0., b_xtg(tile)%data(:,1,n+itracdu-1) )
...
subroutine seasalt(land,fracice,zmid,pblh,v10m,tile,imax) !Inputs
use aerodata_m, only : b_ssn
...
do k=1,kl
  where ( .not.land .and. zmid(:,k)<pblh )
    b_ssn(tile)%data(:,k,1) = ssn_base(:,1)
    b_ssn(tile)%data(:,k,2) = ssn_base(:,2)
  elsewhere ( .not.land )
    b_ssn(tile)%data(:,k,1) = 1.e7*exp(-zmid(:,k)/3000.)
    b_ssn(tile)%data(:,k,2) = 1.e6*exp(-zmid(:,k)/1500.)
  elsewhere
    b_ssn(tile)%data(:,k,1) = 0.
    b_ssn(tile)%data(:,k,2) = 0.
  end where
  ! Reduce over sea ice...
  b_ssn(tile)%data(:,k,1) = (1.-fracice(:))*b_ssn(tile)%data(:,k,1)
  b_ssn(tile)%data(:,k,2) = (1.-fracice(:))*b_ssn(tile)%data(:,k,2)
end do

! These relations give ssm in kg/m3 based on ssn in m^{-3}...
!ssm(tile)%data(:,:,1)=b_ssn(tile)%data(:,:,1)/saltsmallmtn
!ssm(tile)%data(:,:,2)=b_ssn(tile)%data(:,:,2)/saltlargemtn
