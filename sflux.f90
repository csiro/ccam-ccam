...
do tile = 1,ntiles
  ...
  if ( lufull(tile)>0 ) then
    ...
    allocate(lf_intm(tile)%depth(1,nl),lf_intm(tile)%lambda(1,nl),lf_intm(tile)%volcp(1,nl))
    allocate(lf_road(tile)%depth(1,nl),lf_road(tile)%lambda(1,nl),lf_road(tile)%volcp(1,nl))
    allocate(lf_road(tile)%emiss(1),lf_road(tile)%alpha(1))
    allocate(lf_roof(tile)%depth(1,nl),lf_roof(tile)%lambda(1,nl),lf_roof(tile)%volcp(1,nl))
    allocate(lf_roof(tile)%emiss(1),lf_roof(tile)%alpha(1))
    allocate(lf_slab(tile)%depth(1,nl),lf_slab(tile)%lambda(1,nl),lf_slab(tile)%volcp(1,nl))
    allocate(lf_slab(tile)%emiss(1))
    allocate(lf_wall(tile)%depth(1,nl),lf_wall(tile)%lambda(1,nl),lf_wall(tile)%volcp(1,nl))
    allocate(lf_wall(tile)%emiss(1),lf_wall(tile)%alpha(1))
    allocate(lintm(tile)%nodetemp(1,0:nl),lintm(tile)%storage(1,nl))
    allocate(lrdhyd(tile)%surfwater(1),lrdhyd(tile)%snow(1),lrdhyd(tile)%den(1))
    allocate(lrdhyd(tile)%snowalpha(1))
    allocate(lrdhyd(tile)%leafwater(1),lrdhyd(tile)%soilwater(1))
    allocate(lrfhyd(tile)%surfwater(1),lrfhyd(tile)%snow(1),lrfhyd(tile)%den(1))
    allocate(lrfhyd(tile)%snowalpha(1))
    allocate(lrfhyd(tile)%leafwater(1),lrfhyd(tile)%soilwater(1))
    allocate(lrfveg(tile)%emiss(1),lrfveg(tile)%sigma(1),lrfveg(tile)%alpha(1))
    allocate(lrfveg(tile)%zo(1),lrfveg(tile)%lai(1),lrfveg(tile)%rsmin(1))
    allocate(lrfveg(tile)%temp(1))
    allocate(lroad(tile)%nodetemp(1,0:nl),lroad(tile)%storage(1,nl))
    allocate(lroof(tile)%nodetemp(1,0:nl),lroof(tile)%storage(1,nl))
    allocate(lroom(tile)%nodetemp(1,1),lroom(tile)%storage(1,1))
    allocate(lslab(tile)%nodetemp(1,0:nl),lslab(tile)%storage(1,nl))
    allocate(lwalle(tile)%nodetemp(1,0:nl),lwalle(tile)%storage(1,nl))
    allocate(lwallw(tile)%nodetemp(1,0:nl),lwallw(tile)%storage(1,nl))
    allocate(lcnveg(tile)%emiss(1),lcnveg(tile)%sigma(1),lcnveg(tile)%alpha(1))
    allocate(lcnveg(tile)%zo(1),lcnveg(tile)%lai(1),lcnveg(tile)%rsmin(1))
    allocate(lcnveg(tile)%temp(1))
    allocate(lf_industryfg(tile)%data(1), lp_bldheat(tile)%data(1))
    allocate(lp_bldcool(tile)%data(1), lp_traf(tile)%data(1))
    allocate(lp_intgains_full(tile)%data(1), lsigmau(tile)%data(1))
    allocate(lp_cndzmin(tile)%data(1), lp_lzom(tile)%data(1))
    allocate(lp_lzoh(tile)%data(1), lp_cdtq(tile)%data(1))
    allocate(lp_cduv(tile)%data(1), lp_snowmelt(tile)%data(1))
    allocate(lf_bldheight(tile)%data(1), lf_bldwidth(tile)%data(1))
    allocate(lf_coeffbldheight(tile)%data(1), lf_ctime(tile)%data(1))
    allocate(lf_effhwratio(tile)%data(1), lf_fbeam(tile)%data(1))
    allocate(lf_hangle(tile)%data(1), lf_hwratio(tile)%data(1))
    allocate(lf_intgains_flr(tile)%data(1), lf_intmassn(tile)%data(1))
    allocate(lf_rfvegdepth(tile)%data(1), lf_sfc(tile)%data(1))
    allocate(lf_sigmabld(tile)%data(1), lf_ssat(tile)%data(1))
    allocate(lf_swilt(tile)%data(1), lf_trafficfg(tile)%data(1))
    allocate(lf_vangle(tile)%data(1), lp_emiss(tile)%data(1))
    allocate(lp_atmoserr(tile)%data(1), lp_surferr(tile)%data(1))
    allocate(lint_psi(tile)%data(1,4,4), lint_viewf(tile)%data(1,4,4))
    allocate(lp_qscrn(tile)%data(1), lp_tscrn(tile)%data(1))
    allocate(lp_u10(tile)%data(1), lp_uscrn(tile)%data(1))
    allocate(lf_infilach(tile)%data(1), lf_ventilach(tile)%data(1))
    allocate(lf_tempcool(tile)%data(1), lf_tempheat(tile)%data(1))
    allocate(lf_bldairtemp(tile)%data(1))
  end if
