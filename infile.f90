...
if ( vtype==nf90_short ) then
  ...
  if ( procformat ) then
     if ( useiobuffer ) then
        call add_iobuffer(sname,lidnc,mid,vtype,ndims,ifull,istep,nproc_node,start,ncount,ipack=ipack)
     else
  ...
  if ( procformat ) then
     if ( useiobuffer ) then
        call add_iobuffer(sname,lidnc,mid,vtype,ndims,ifull,istep,nproc_node,start,ncount,var=var)
     else
...
if ( vtype==nf90_short ) then
  ...
  if ( procformat ) then
     if ( useiobuffer ) then
        call add_iobuffer(sname,lidnc,mid,vtype,ndims,ifull,kl,nproc_node,start,ncount,ipack=ipack)
     else
  ...
  if ( procformat ) then
     if ( useiobuffer ) then
        call add_iobuffer(sname,lidnc,mid,vtype,ndims,ifull,kl,nproc_node,start,ncount,var=var)
     else
...
if ( procformat ) then
   if ( myid_node.eq.0 ) then
      ...
      ncstatus = nf90_create("/tmp/"//trim(fname),nf90_netcdf4,lncid)
   end if
else
   ncstatus = nf90_create(fname,mode,lncid)
end if
