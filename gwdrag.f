...
!     this is vectorized jlm version
      ...
      use soil_m
      implicit none
      !real, parameter :: fc2 = 0.5   !  as per Hal      gwd1a
      !integer, parameter :: ndzx = 0 !  as per Hal      gwd1a
      real, parameter :: fc2 = 1.     !  as per jlm      gwd1b
...
!     calculate bvnf at other levels,  (Brunt-Vaisala-N-full)
      do k=1,kl-1
        bvnf(:,k)=sqrt( max(1.e-20,
     &      grav*(dthdz(:,k)+dthdz(:,k+1))/(thf(:,k)+thf(:,k+1))) ) ! jlm fixup
      enddo    ! k loop
...
!       calc max(1-fc**2/f**2,0) : put in fni()
        fni(:,k)=max(0. , 1.-
     &   sig(k)*temp(:)*uu(:,k)**3/(sigk(k)*bvnf(:,k)*thf(:,k)))
      enddo    ! k loop
