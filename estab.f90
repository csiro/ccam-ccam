...
tstore = tdiff(t_)
do iq = 1,size(t_)
  tfrac = tstore(iq) - aint(tstore(iq))
  tpos = int(tstore(iq))
  ! Arithmetic statement functions to replace call to establ.
  ! T is temp in Kelvin, which should lie between 123.16 and 343.16;
...
real, dimension(:,:), intent(in) :: t_
real, dimension(size(t_,1),size(t_,2)) :: ans
real, dimension(size(t_,1),size(t_,2)) :: tstore
real tfrac
integer tpos
real, dimension(0:220), parameter :: table = &
(/ 1.e-9, 1.e-9, 2.e-9, 3.e-9, 4.e-9,                                    & !-146C
...
do k = 1,size(t_,2)
  do iq = 1,size(t_,1)
    tfrac = tstore(iq,k) - aint(tstore(iq,k))
    tpos = int(tstore(iq,k))
...
real, dimension(:,:), intent(in) :: pp_, t_
real, dimension(size(t_,1),size(t_,2)) :: ans
real, dimension(size(t_,1),size(t_,2)) :: estore
estore = establ(t_)
ans = epsil*estore/max(pp_-estore,.1) !jlm strato
! ans = epsil*establ(t_)/(pp_-establ(t_)) !Usual formula
! ans = epsil*establ(t_)/pp_ !Consistent with V4-5 to V4-7
...
real, dimension(:), intent(in) :: tx_
real, dimension(size(tx_)) :: ans
real, dimension(size(tx_)) :: tstore
real tfrac
integer tpos
real, dimension(-40:2), parameter :: esdiff= &
(/ 6.22, 6.76, 7.32, 7.92, 8.56, 9.23, 9.94,10.68,11.46,12.27,  &
...
real, dimension(:,:), intent(in) :: tx_
real, dimension(size(tx_,1),size(tx_,2)) :: ans
real, dimension(size(tx_,1),size(tx_,2)) :: tstore
real tfrac
integer tpos
real, dimension(-40:2), parameter :: esdiff= &
(/ 6.22, 6.76, 7.32, 7.92, 8.56, 9.23, 9.94,10.68,11.46,12.27,  &
