...
ans = 610.94*exp(17.625*(t_-273.16)/(t_-273.16+243.04))  ! Magnus
...
ans = 611.21*exp(22.587*(t_-273.16)/(t_-273.16+273.86))  ! Magnus
...
pure function esdiffx_s(t_) result(ans)
!$acc routine vector
implicit none
!integer tpos
real, intent(in) :: t_
...
a = 610.94*exp(17.625*(t_-273.16)/(t_-273.16+243.04))  ! Magnus
b = 611.21*exp(22.587*(t_-273.16)/(t_-273.16+273.86))  ! Magnus
ans = a - b

end function esdiffx_s

pure function esdiffx_v(t_,imax_) result(ans)
!$acc routine vector
implicit none
integer, intent(in) :: imax_
real, dimension(imax_), intent(in) :: t_
real, dimension(imax_) :: ans
real, dimension(imax_) :: a, b
!real, dimension(imax_) :: tstore, tfrac
!integer, dimension(imax_) :: tpos
!real, dimension(-40:2), parameter :: esdiff= &
!(/ 6.22, 6.76, 7.32, 7.92, 8.56, 9.23, 9.94,10.68,11.46,12.27,  &
