...
!     REAL(r_1), DIMENSION(:), POINTER :: cnsdJ ! thermal conductivity of dry soil [W/m/K]
  ...
  TYPE soil_snow_type 
     ...
     REAL(r_2), DIMENSION(:,:), POINTER :: evapfbl !  water extracted by canopy
     REAL(r_1), DIMENSION(:), POINTER :: sinfil   ! water flux to the soil
...
!     REAL(r_1), DIMENSION(:,0:3), POINTER :: smelt1 ! snow melt (3 layer snowpack) 
     ...
     REAL(r_1), DIMENSION(:), POINTER :: wb_lake   ! total soil water (mm)
...
!     REAL(r_1), DIMENSION(:), POINTER :: hc_grdmax ! maximum height of canopy from tiles belonging to the same grid 
     ...
     REAL(r_1), DIMENSION(:), POINTER :: extkn  ! kdcorbin, 10/10
  ...
  TYPE canopy_type
     ...
     REAL(r_1), DIMENSION(:), POINTER :: ua_10m ! 10m wind speed (m/s)
...
!jhan:#define jhntiles in cable_directives.h    
    ALLOCATE ( var % tilefrac(mp,JHNTILES) )
  ...
  SUBROUTINE alloc_veg_parameter_type(var, mp)
    ...
    ALLOCATE ( var % vegcf(mp) ) !kdcorbin, 08/10
    ALLOCATE ( var % extkn(mp) ) !kdcorbin, 10/10 
...
!    DEALLOCATE ( var % cnsdJ )
  ...
  SUBROUTINE dealloc_veg_parameter_type(var, mp)
    ...
    DEALLOCATE ( var % vegcf )  !kdcorbin, 08/10
    DEALLOCATE ( var % extkn )  !kdcorbin, 10/10
