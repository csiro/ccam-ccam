MODULE radiation_module
  ! use Goudriaan's radiation scheme for calculate radiation absorbed by 
  ! canopy 
  ...
  SUBROUTINE init_radiation(met,rad,veg)
    ...
    WHERE (veg%vlaiw > 1e-2) ! vegetated
       ...
       rad%extkd = -LOG(SUM(SPREAD(gauss_w, 1, mp) * &
           EXP(-xk * xvlai2), 2)) / veg%vlaiw
    ELSEWHERE ! i.e. bare soil
       rad%extkd = 0.7
    END WHERE
    ! corrections for vegetation 1 and 4 commented out for a time beeing by EAK

    ! Extinction coefficient for leaf nitrogen profile in canopy:
    ! rad%extkn = rad%extkd * SQRT(1.0 - veg%taul(:,1) - veg%refl(:,1))
!    rad%extkn = 0.7
!    where( veg%iveg == 2 ) rad%extkn = 0.01
    rad%extkn = 0.001
...
!       rad%extkb=1.0e5
  ...
  !---------------------------------------------------------------------
  SUBROUTINE radiation(ktau,bal, soil, ssoil, veg, air, met, rad)
    ...
    REAL(r_1), DIMENSION(mp)  :: cf1    ! (1.0 - transb * cexpkdm) / 
                                        !   (extkb + extkdm(:,b))
    REAL(r_1), DIMENSION(mp)  :: cf3    ! (1.0 - transb * cexpkbm) / 
                                        !   (extkb + extkbm(:,b))
    ...
    REAL(r_1), DIMENSION(mp)  :: flwv   ! vegetation long-wave radiation 
                                        !    (isothermal)
    ...
    !kdcorbin, 10/10 - using extkn from parameter file
    cf2n = EXP(-veg%extkn * veg%vlaiw)
    rad%transd = 1.0
    WHERE (veg%vlaiw > 1e-2)    ! In gridcells where vegetation exists....
    ...
    WHERE (veg%vlaiw > 1.e-2)
       ...
       rad%gradis(:,2) = (8.0 * emleaf / (capp * air%rho)) * flpwb / &
            (met%tk) * &
       ...
       WHERE (mask) ! i.e. vegetation and sunlight are present
          ...
          rad%qcan(:,2,b) = met%fsd(:,b) * ( &  ! scale to real shaded flux
               ...
               * ((1.0 - rad%cexpkbm(:,b)) / rad%extkbm(:,b) - cf3) &
               - rad%fbeam(:,b)*(1.0-veg%taul(:,b)-veg%refl(:,b))*rad%extkb &
               * ((1-rad%transb)/rad%extkb - (1-rad%transb**2)/ &
                    (rad%extkb+rad%extkb)))
       END WHERE
    END DO
    ...
    WHERE (mask) ! i.e. vegetation and sunlight are present
       ...
       rad%qssabs = met%fsd(:,1) * (    &
          ...
          EXP(-rad%extkbm(:,1)*veg%vlaiw) +  &
             (1.-rad%fbeam(:,1))*(1.-rad%reffdf(:,1))* &
          EXP(-rad%extkdm(:,1)*veg%vlaiw) ) + &
  ...
  ELEMENTAL FUNCTION sinbet(doy,xslat,hod) RESULT(z)
    ...
    z = MAX( &
         SIN(pi180 * xslat) * sindec + COS(pi180 * xslat) * &
  ...
  !-------------------------------------------------------------------
   FUNCTION spitter(doy, coszen, fsd) RESULT(fbeam)
    ...
    REAL(r_1), DIMENSION(mp), INTENT(IN) :: fsd ! short wave down 
                                                !   (positive) w/m^2
