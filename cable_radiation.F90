...
!    print *,'initrad',veg%vlaiw,xvlai2,veg%xfang
...
!    print *,'initrad4',rad%extkn,taul(1),refl(1),c1
!    print *,'initrad5',rhoch,rad%rhocdf
...
!    print *,'rad8a', b,rad%extkb,rad%extkdm,rad%extkbm
       ...
       WHERE (mask) ! i.e. vegetation and sunlight are present
          ...
          rad%qcan(:,1,b) = met%fsd(:,b) * ( &		! scale to real sunlit flux
               ...
               + rad%fbeam(:,b)*(1.0-taul(b)-refl(b))*rad%extkb &
...
101 format(1x,'radalbedo',i6,8f6.3,2f5.2,2f6.3)
    ...
    WHERE (mask) ! i.e. vegetation and sunlight are present
       ...
       rad%qssabs = met%fsd(:,1) * ( &       
            ...
            met%fsd(:,2)*(                                                               &
            rad%fbeam(:,2)*(1.-rad%reffbm(:,2))*rad%cexpkbm(:,2) +(1.-rad%fbeam(:,2))*(1.-rad%reffdf(:,2))*rad%cexpkdm(:,2))
