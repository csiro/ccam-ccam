...
integer, parameter :: nfreq_bands_sea_all = nfreq_bands_sea_ch4+nfreq_bands_sea_n2o+nfreq_bands_sea_co2
...
!--------------------------------------------------------------------
              if (ch4_vmr /= 0.0) then
                call gasins(gas_type, do_lvlcalc_ch4, do_lvlctscalc_ch4,      &
                            ...
                            dgasdt10_lvl(:,:,1), dgasdt10_lvlcts(:,1), dgasdt10_lyr(:,:,nf_offset),      &
                            gasp10_lvl(:,:,1), gasp10_lvlcts(:,1), gasp10_lyr(:,:,nf_offset),            &
                            d2gast10_lvl(:,:,1), d2gast10_lvlcts(:,1), d2gast10_lyr(:,:,nf_offset),      &
                            dgasdt8_lvl(:,:,1),  dgasdt8_lvlcts(:,1),  dgasdt8_lyr(:,:,nf_offset) ,      &
                            gasp8_lvl(:,:,1),  gasp8_lvlcts(:,1),  gasp8_lyr(:,:,nf_offset) ,            &
                            d2gast8_lvl(:,:,1),  d2gast8_lvlcts(:,1),  d2gast8_lyr(:,:,nf_offset) )
...
!-------------------------------------------------------------------
        do k=nklo,nkhi-1
          do kp=1,25
            pressnbl_lov(kp,k) = pd(k-1) + real(kp-1)*  &
                                 (pd(k) - pd(k-1))/50.
          ...
          do kp=27,51
            pressnbl_hiv(kp,k) = pd(k-1) + real(kp-1)*  &
                                 (pd(k) - pd(k-1))/50.
            pressnbl_lov(kp,k) = plm(k)
          enddo
        enddo
        do kp=1,50
          pressnbl_lov(kp,nkhi) = pd(nkhi-1) + real(kp-1)*  &
...
!-------------------------------------------------------------------        
        do k=nklo,nkhi-1
          do kp=1,25
            pressnbl_lov(kp,k) = pd8(k-1) + real(kp-1)*  &
                                 (pd8(k) - pd8(k-1))/50.
          ...
          do kp=27,51
            pressnbl_hiv(kp,k) = pd8(k-1) + real(kp-1)*  &
                                 (pd8(k) - pd8(k-1))/50.
            pressnbl_lov(kp,k) = plm8(k)
          enddo
        enddo
        do kp=1,50
          pressnbl_lov(kp,nkhi) = pd8(nkhi-1) + real(kp-1)*  &
