...
module cu_gf_deep
     ...
     real, save         :: qamin =0.0004 
     ! use machine , only : kind_phys                              ! sny
     ! use physcons, only : qamin                                  ! sny
...
!!$acc declare copy(mconv,ccn)
     ...
     real(kind=kind_phys), intent (in) :: fscav(nchem)
...
!!$acc parallel loop private(beta,itemp,dzo,h_entr)
      do i=its,itf
        ...
        dd_massdetro(i,:)=0.
        dd_massentro(i,:)=0.
        tmp_po_cup(:) = po_cup(i,:)  ! MJT - for array tmpry
        tmp_zdo(:) = zdo(i,:)        ! MJT - for array tmpry
        call get_zu_zd_pdf_fim(0,tmp_po_cup,rand_vmas(i),0.0_kind_phys,ipr,xland1(i),zuh2,4, &
            ierr(i),kdet(i),jmin(i)+1,tmp_zdo,kts,kte,ktf,beta,kpbl(i),csum(i),pmin_lev(i))
        zdo(i,:) = tmp_zdo           ! MJT - for array tmpry
...
! MJT notes - code below needed refactoring for ifx compiler..........
...
! MJT notes - code above needed refactoring for ifx compiler..........
...
!
     ...
     real(kind=kind_phys),    dimension (its:ite,kts:kte) :: zu_temp     
...
!
! modi_1 : c0t larger by 5 : c0t     = c0t*5.
! modi_2 : qrc larger by 5 : qrc(i,k)= qrc(i,k)*5.
! modi_3 : zu larger by 5  : zu(i,k) = zu(i,k)*5.
!                zu_temp(i,k) = zu(i,k)*5.                
!                pw(i,k)=c0t*dz*qrc(i,k)*zu_temp(i,k)
! end modification
...
!> Driver for the normalized mass-flux routine.
 subroutine rates_up_pdf(rand_vmas,ipr,name,ktop,ierr,p_cup,entr_rate_2d,hkbo,heo,heso_cup,z_cup, &
     ...
     real(kind=kind_phys), dimension(kts:kte) :: tmp_p_cup, tmp_zuo ! MJT - for array tmpry
...
!        ktop(i)=kfinalzu
        if(kfinalzu.le.kbcon(i)+2)then
           ...
           call get_zu_zd_pdf_fim(kklev,tmp_p_cup,rand_vmas(i),zubeg,ipr,xland(i),zuh2,1,ierr(i),k22(i), &
            kfinalzu+1,tmp_zuo,kts,kte,ktf,beta_u,kbcon(i),csum(i),pmin_lev(i))
      ...
      if ( is_shallow ) then
       if(ktop(i) <= kbcon(i)+2)then
           ...
           tmp_p_cup(:) = p_cup(i,:)   ! MJT - for array tmpry
           tmp_zuo(:) = zuo(i,kts:kte) ! MJT - for array tmpry
           call get_zu_zd_pdf_fim(kbcon(i),tmp_p_cup,rand_vmas(i),zubeg,ipr,xland(i),zuh2,2,ierr(i),k22(i), &
             ktopdby(i)+1,tmp_zuo,kts,kte,ktf,beta_u,kbcon(i),csum(i),pmin_lev(i))
           zuo(i,kts:kte) = tmp_zuo(:) ! MJT - for array tmpry
         endif
         endif ! shal
     enddo
