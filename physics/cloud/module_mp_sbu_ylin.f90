...
!---- new snow parameters
  ...
  !$acc parallel loop collapse(2) copyin(tem,viscmu,Ri,rho)              &
  !$acc   copyout(n0_s,gam_bv_s,am_s,bm_s,aa_s,av_s,bv_s,tmp_sa,gam_ss)  &
  !$acc   copyout(gam_bm_s,gam_bv_ss)
  do k = kts, kte
    do iq = 1,imax
  ...
  !$acc parallel loop collapse(2) copy(ncz,niz,nrz,nsz,thz,qvz,qlz,qiz,qsz,qrz,Ri)  &
  !$acc   copyin(n0_s,tem,prez,tothz,rho,dzw,viscmu,am_s,bm_s,aa_s,av_s,bv_s)       &
  !$acc   copyin(tmp_sa,gam_ss,gam_bm_s,gam_bv_ss)                                  &
  !$acc   copyout(fluxr,fluxi,fluxs,fluxm,fluxf,fevap,fsubl,fauto,fcoll,faccr)      &
  !$acc   copyout(effc1d,effr1d,effs1d,effi1d)                                      &
  do k = kts,kte
    do iq = 1,imax
