...
subroutine clphy1d_ylin(dt_in, ifull, imax,                 &
                      qvz, qlz, qrz, qiz, qsz,              &
                      thz, tothz, rho,                      &
                      prez, zz, dzw,                        &
...
! new 2D declaration
  ...
  real,                             intent(in)    :: dt_in, maxlintime
  real, dimension(1:ifull,kts:kte), intent(in)    :: zdrop
  real, dimension(1:ifull,kts:kte), intent(inout) :: Ri
  real, dimension(1:ifull,kts:kte), intent(in)    :: tothz,rho,prez,dzw
  real, dimension(1:ifull,0:kte),   intent(in)    :: zz
  real, dimension(1:ifull,kts:kte), intent(out)   :: EFFC1D,EFFI1D,                     &
                                                     EFFS1D,EFFR1D
  real, dimension(1:ifull,kts:kte), intent(out)   :: fluxr,fluxi,fluxs,                 &
                                                     fluxm,fluxf,fevap,fsubl,           &
                                                     fauto,fcoll,faccr
  real, dimension(1:ifull,kts:kte), intent(out)   :: vi
#ifndef GPU  
  real, dimension(1:ifull,kts:kte), intent(out)   :: zpsnow,zpsaut,zpsfw,               &
                                                     zpsfi,zpraci,zpiacr,               &
                                                     ...
                                                     zqschg
#endif  
  real, dimension(1:ifull),         intent(inout) :: pptrain, pptsnow, pptice
  real, dimension(1:ifull,kts:kte), intent(inout) :: qvz,qlz,qrz,qiz,qsz,thz
  real, dimension(1:ifull,kts:kte), intent(inout) :: ncz,niz,nrz,nsz
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  integer                                        :: k, iq
  integer                                        :: ntiles, tile, js, je
  integer                                        :: njumps, n
  ! local vars
  real                                           :: dt
  ...
  real, dimension(1:ifull,kts:kte)               :: tem
  real                                           :: qswz, qsiz
  real                                           :: theiz, temcc
...
!---- new snow parameters
  ...
  ntiles = ifull/imax  
  njumps = int(dt_in/(maxlintime+0.01)) + 1
  ...
  do n = 1,njumps
    ...
    do tile = 1,ntiles
      ...
      select case( lin_aerosolmode )
        case(0)
          do k=kts,kte
            ...
            ncz(js:je,k) = Nt_c/rho(js:je,k)
          end do
        case(1)
          do k=kts,kte
            ncz(js:je,k) = zdrop(js:je,k)/rho(js:je,k)
          end do
        case default
      ...
      do k = kts,kte
        niz(js:je,k) = min(niz(js:je,k),0.3E6/rho(js:je,k))
      ...
      !
      do k = kte-1,kts,-1
        Ri(js:je,k) = max( Ri(js:je,k), Ri(js:je,k+1) )
      end do
    ...
    do tile = 1,ntiles
      do k = kts, kte
        ...
        do iq = js,je
          ...
          am_s(iq,k) = am_c1+am_c2*tc0+am_c3*Ri(iq,k)**2          !--- Heymsfield 2007
          ...
          if ( mask ) then
            ...
            if (tem(iq,k) < 273.15) then
              ...
              if(qrz(iq,k) > 0.) then  ! go to 1000
                ...
                tmp1 = olambdar**bp3 
                npiacr=pio4*eri*nrz(iq,k)*av_r*niz(iq,k)*gambp3* &
                      tmp1 !--wdm6
              ...
              if(qsz(iq,k) > 0.) then !go to 1200
                ...
                tmp2= abi*n0_s(iq,k)*( vf1s*olambdas**2+ &
