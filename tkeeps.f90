...
do k = 1,kl
  ...
  b_tke(tile)%data(:,k) = max(b_tke(tile)%data(:,k), mintke)
  tff(:) = cm34*b_tke(tile)%data(:,k)*sqrt(b_tke(tile)%data(:,k))
  b_eps(tile)%data(:,k) = min(b_eps(tile)%data(:,k), tff/minl)
  b_eps(tile)%data(:,k) = max(b_eps(tile)%data(:,k), tff/maxl, mineps)
  ...
  km(:,k) = cm0*b_tke(tile)%data(:,k)*b_tke(tile)%data(:,k)/b_eps(tile)%data(:,k)
end do
...
pps(:,2:kl-1) = km(:,2:kl-1)*b_shear(tile)%data(:,2:kl-1)
...
do kcount = 1,mcount
  ...
  mf(tile)%data(:,:)=0.
  w_up(tile)%data(:,:)=0.
  tl_up(tile)%data(:,:)=thetal(1:imax,:)
  qv_up(tile)%data(:,:)=qvg(1:imax,:)
  ql_up(tile)%data(:,:)=qlg(1:imax,:)
  qf_up(tile)%data(:,:)=qfg(1:imax,:)
  cf_up(tile)%data(:,:)=0.
  ents(tile)%data(:,:)=0.
  dtrs(tile)%data(:,:)=0.
#endif
  ...
  if ( mode/=1 ) then ! mass flux
    ...
    do i = 1,imax
      ...
      if ( wtv0(i)>0. ) then ! unstable
        ! Initialise updraft
        b_tke(tile)%data(i,1)=cm12*ustar(i)*ustar(i)+ce3*wstar(i)*wstar(i)
        b_tke(tile)%data(i,1)=max(b_tke(tile)%data(i,1),mintke)
        ktopmax=0
        w2up=0.
        dzht=zz(i,1)
        ...
        tlup(i,1)=thetal(i,1)+be*wt0(i)/sqrt(b_tke(tile)%data(i,1))       ! Hurley 2007
        qvup(i,1)=qvg(i,1)   +be*wq0(i)/sqrt(b_tke(tile)%data(i,1))       ! Hurley 2007
        qlup(i,1)=qlg(i,1)
        qfup(i,1)=qfg(i,1)
        ...
        nn(1)  =grav*be*wtv0(i)/(thetav(i,1)*sqrt(b_tke(tile)%data(i,1))) ! Hurley 2007
        ...
        do k=2,kl
          ...
          sigqtup=sqrt(max(1.E-10, 1.6*b_tke(tile)%data(i,k)/b_eps(tile)%data(i,k)*cq*km(i,k)*((qtup(k)-qtup(k-1))/dzht)**2))
...
#ifdef offline
        do k=1,ktopmax
          mf(tile)%data(i,k)=mflx(i,k)
          w_up(tile)%data(i,k)=sqrt(w2up(k))
          tl_up(tile)%data(i,k)=tlup(i,k)
          qv_up(tile)%data(i,k)=qvup(i,k)
          ql_up(tile)%data(i,k)=qlup(i,k)
          qf_up(tile)%data(i,k)=qfup(i,k)
          cf_up(tile)%data(i,k)=cfup(i,k)*min(mflx(i,k)/sqrt(w2up(k)),1.)
        end do
        ...
        dtr = -1./dzht + ent
        dtr = max( dtr, 0. )
        ents(tile)%data(i,1) = ent
        dtrs(tile)%data(i,1) = dtr
        do k = 2,ktopmax
          dzht = dz_hl(i,k-1)
          ! Entrainment and detrainment rates
          ent = entfn(zz(i,k),zi(i))
          ...
          dtr = max( dtr, 0. )
          ents(tile)%data(i,k) = ent
          dtrs(tile)%data(i,k) = dtr
        end do
#endif
  ...
  b_tke(tile)%data(:,1)=cm12*ustar*ustar+ce3*wstar*wstar
  b_eps(tile)%data(:,1)=ustar*ustar*ustar*phim/(vkar*zz(:,1))+grav*wtv0/thetav(:,1)
  b_tke(tile)%data(:,1)=max(b_tke(tile)%data(:,1),mintke)
  tff=cm34*b_tke(tile)%data(:,1)*sqrt(b_tke(tile)%data(:,1))
  b_eps(tile)%data(:,1)=min(b_eps(tile)%data(:,1),tff/minl)
  b_eps(tile)%data(:,1)=max(b_eps(tile)%data(:,1),tff/maxl,mineps)


  ! Update TKE and eps terms

  ! top boundary condition to avoid unphysical behaviour at the top of the model
  b_tke(tile)%data(:,kl)=mintke
  b_eps(tile)%data(:,kl)=mineps
  ...
  do k=2,kl-1
    ppt(:,k)= kmo(:,k)*idzp(:,k)*(b_tke(tile)%data(:,k+1)-b_tke(tile)%data(:,k))/dz_hl(:,k)                &
             -kmo(:,k-1)*idzm(:,k)*(b_tke(tile)%data(:,k)-b_tke(tile)%data(:,k-1))/dz_hl(:,k-1)
  end do
  ...
  bb(:,2:kl-1)=1.-aa(:,2:kl-1)-cc(:,2:kl-1)+ddts*ce2*b_eps(tile)%data(:,2:kl-1)/b_tke(tile)%data(:,2:kl-1)
  dd(:,2:kl-1)=b_eps(tile)%data(:,2:kl-1)+ddts*b_eps(tile)%data(:,2:kl-1)/b_tke(tile)%data(:,2:kl-1)                    &
              *ce1*(pps(:,2:kl-1)+max(ppb(:,2:kl-1),0.)+max(ppt(:,2:kl-1),0.))
  dd(:,2)     =dd(:,2)   -aa(:,2)*b_eps(tile)%data(:,1)
  dd(:,kl-1)  =dd(:,kl-1)-cc(:,kl-1)*mineps
  call thomas(b_eps(tile)%data(:,2:kl-1),aa(:,3:kl-1),bb(:,2:kl-1),cc(:,2:kl-2),dd(:,2:kl-1),imax)

  ! TKE vertical mixing
  aa(:,2:kl-1)=kmo(:,1:kl-2)*qq(:,2:kl-1)
  cc(:,2:kl-1)=kmo(:,2:kl-1)*rr(:,2:kl-1)
  bb(:,2:kl-1)=1.-aa(:,2:kl-1)-cc(:,2:kl-1)
  dd(:,2:kl-1)=b_tke(tile)%data(:,2:kl-1)+ddts*(pps(:,2:kl-1)+ppb(:,2:kl-1)-b_eps(tile)%data(:,2:kl-1))
  dd(:,2)     =dd(:,2)   -aa(:,2)*b_tke(tile)%data(:,1)
  dd(:,kl-1)  =dd(:,kl-1)-cc(:,kl-1)*mintke
  call thomas(b_tke(tile)%data(:,2:kl-1),aa(:,3:kl-1),bb(:,2:kl-1),cc(:,2:kl-2),dd(:,2:kl-1),imax)
  ...
  if ( tkemeth==1 ) then
    do k = 2,kl-1
      ...
      where ( wstar(:)>0.5 .and. zz(:,k)>0.5*zi(:) .and. zz(:,k)<0.95*zi(:)   )
        b_tke(tile)%data(:,k) = max( b_tke(tile)%data(:,k), tbb(:)*b_tke(tile)%data(:,k-1) )
        b_eps(tile)%data(:,k) = max( b_eps(tile)%data(:,k), tbb(:)*b_eps(tile)%data(:,k-1) )
      end where
    end do
  end if
  ...
  km=cm0*b_tke(tile)%data(:,:)*b_tke(tile)%data(:,:)/b_eps(tile)%data(:,:)
...
#endif
#ifdef offline
  wthl(tile)%data(:,1:kl-1)=-kmo(:,1:kl-1)*(thetal(1:imax,2:kl)-thetal(1:imax,1:kl-1))/dz_hl(:,1:kl-1)                    &
                 +mflx(:,1:kl-1)*(tlup(:,1:kl-1)-thetal(:,1:kl-1))*(1.-fzzh(:,1:kl-1))                           &
                 +mflx(:,2:kl)*(tlup(:,2:kl)-thetal(:,2:kl))*fzzh(:,1:kl-1)
#endif
...
#endif
#ifdef offline
  wqv(tile)%data(:,1:kl-1)=-kmo(:,1:kl-1)*(qvg(1:imax,2:kl)-qvg(1:imax,1:kl-1))/dz_hl(:,1:kl-1)                           &
                 +mflx(:,1:kl-1)*(qvup(:,1:kl-1)-qvg(:,1:kl-1))*(1.-fzzh(:,1:kl-1))                              &
                 +mflx(:,2:kl)*(qvup(:,2:kl)-qvg(:,2:kl))*fzzh(:,1:kl-1)
#endif
...
#endif
#ifdef offline
  wql(tile)%data(:,1:kl-1)=-kmo(:,1:kl-1)*(qlg(1:imax,2:kl)-qlg(1:imax,1:kl-1))/dz_hl(:,1:kl-1)                           &
                 +mflx(:,1:kl-1)*(qlup(:,1:kl-1)-qlg(:,1:kl-1))*(1.-fzzh(:,1:kl-1))                              &
                 +mflx(:,2:kl)*(qlup(:,2:kl)-qlg(:,2:kl))*fzzh(:,1:kl-1)
#endif
...
#endif
#ifdef offline
  wqf(tile)%data(:,1:kl-1)=-kmo(:,1:kl-1)*(qfg(1:imax,2:kl)-qfg(1:imax,1:kl-1))/dz_hl(:,1:kl-1)                           &
                 +mflx(:,1:kl-1)*(qfup(:,1:kl-1)-qfg(:,1:kl-1))*(1.-fzzh(:,1:kl-1))                              &
                 +mflx(:,2:kl)*(qfup(:,2:kl)-qfg(:,2:kl))*fzzh(:,1:kl-1)
#endif
  ...
  ! dd(:,1:kl)=uo(1:imax,1:kl)-ddts*taux/(rhoa(:,1)*dz_fl(:,1)) ! explicit
  call thomas(uo,aa(:,2:kl),bb(:,1:kl),cc(:,1:kl-1),dd(:,1:kl),imax)
  dd(:,1:kl)=vo(1:imax,1:kl)
  ! dd(:,1:kl)=vo(1:imax,1:kl)-ddts*tauy/(rhoa(:,1)*dz_fl(:,1)) ! explicit
  call thomas(vo,aa(:,2:kl),bb(:,1:kl),cc(:,1:kl-1),dd(:,1:kl),imax)
  
  ! umag=sqrt(max(uo(1:imax,1)*uo(1:imax,1)+vo(1:imax,1)*vo(1:imax,1),1.e-4)) ! explicit
  ! call dyerhicks(cdrag,wtv0,zom,umag,thetav(:,1),zz(:,1))                       ! explicit
  ! taux=rhos*cdrag*umag*uo(1:imax,1)                                            ! explicit
  ! tauy=rhos*cdrag*umag*vo(1:imax,1)                                            ! explicit
