...
8     print *,'end of data reading file in infile'
     ...
     & t,u,v,qg,nested)
!     nested=0  for calls from indata; 1  for calls from nestin     
      parameter (nmeth=1)  ! 0 old way, 1 preferred new way (see nestin too)
      include 'newmpar.h'
      include 'darcdf.h'
      include 'extraout.h'
     ...
     & ,ntsuri,nrad2,kuocb2,nvmix2,ntsea2,ms2,nextras2,ilt2,ntrac2
      ...
      common/schmidtx/rlong0x,rlat0x,schmidtx ! infile, newin, nestin, indata
      common/sigin/sigin(kl),kk  ! for vertint, infile
      common/work2/dum0(ifull),tmp(ifull),dum4(ifull,16)

      parameter(nihead=54,nrhead=14)
     ...
     &         ,'jul','aug','sep','oct','nov','dec'/
      ...
      data ncidold/-1/,ncalled/0/
      save iarch,ncalled,ncidold,nqg_setin
      ...
      ncalled=ncalled+1
      ncid=idifil
      print *,'newin ncid,ncidold=',ncid,ncidold
...
c     print *,'call ncagt'
      ...
      rlat0x  =ahead(6)
      schmidtx=ahead(7)
      print *,'newin  rlong0x,rlat0x,schmidtx ',rlong0x,rlat0x,schmidtx
...
c       find i/j in input grid of model grid
     ...
     &           numi,numj,numk
      ...
      if ( numi .gt. il ) stop 'numi > il'
      if ( numj .gt. jl ) stop 'numj > jl'
...
!     common/work3/dont_use(ifull,kl,3),ivar(ijk),spare(ijk)
      ...
      ni=iend-istrt+1
      nj=jend-jstrt+1
      if ( ni*nj.gt.il*jl) stop 'histrd1 ni*nj gt il*jl'
      ...
      vmin= 1.e29
       do jin=jstrt,jend
        ...
        do iin=istrt,iend
          ...
          var(i,j) = ivar(inij)*sf + addoff
          vmax=max(vmax,var(i,j))
          vmin=min(vmin,var(i,j))
        end do
      end do

      if(mod(ktau,nmaxpr).eq.0.or.odiag)
     &   write(6,'("done histrd1 ",a6,i4,i3,3e14.6)')
     .        name,ier,iarch,vmin,vmax,var(id,jd)
...
!     common/work3/dont_use(ifull,kl,3),ivar(ijk),spare(ijk)
      ...
      if ( ik*jk*kk.gt.200*200*35) stop 'histrd4 ik*jk*kk gt 200*200*35'
!     if ( ik*jk*kk.gt.ijk) stop 'histrd4 ik*jk*kk gt ijk'
      ni=iend-istrt+1
      nj=jend-jstrt+1
      if ( ni*nj*numk.gt.il*jl*kl) stop 'histrd4 ni*nj*numk gt il*jl*kl'
...
c unpack data
      ...
      do k=1,numk
       do jin=jstrt,jend
        do iin=istrt,iend
          ...
          i=iin+1-istrt
          j=jin+1-jstrt
          var(i,j,k) = ivar(inijk)*sf + addoff
          vmax=max(vmax,var(i,j,k))
          vmin=min(vmin,var(i,j,k))
        end do ! i
       end do  ! j
      end do   ! k=1,numk
     ...
     .    name,ier,iarch,vmin,vmax,var(id,jd,nlv)
