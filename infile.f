...
c*************************************************************************
     ...
     &  name,ier,iarchi,vmin,vmax,globvar(id+(jd-1)*ik)
      ...
      if (ifull/=ik*jk.and.ptest) then
       ...
       if(ier==0.and.mod(ktau,nmaxpr)==0.and.myid==0)then
        write(6,'("done histrd4 ",a8,i3,i4,i3)')
     &   name,kk,ier,iarchi
       endif
...
!       allow for roundoff for real timer from old runs
     ...
     & (/0,31,59,90,120,151,181,212,243,273,304,334/)      ! days from beginning of year (1st Jan is 0)
      ...
      if (myid==0) then
        ...
        write(6,*) "Found pil_g,pjl_g,fnproc,dmode ",pil_g,pjl_g,
     &              fnproc,dmode
        ...
        select case(dmode)
          ...
          case(1) ! face decomposition
            pnpan=max(1,fnproc/6)
            do ipf=0,fnproc-1
              call face_set(pil,pjl,pnoff(ipf),duma,dumb,
     &               pnpan,pil_g,ipf,fnproc,nxpr,nypr)
              pioff(ipf,:)=duma
              pjoff(ipf,:)=dumb
            end do
          case(2) ! uniform decomposition
            pnpan=6
            do ipf=0,fnproc-1
              call uniform_set(pil,pjl,pnoff(ipf),duma,dumb,
     &               pnpan,pil_g,ipf,fnproc,nxpr,nypr)
              pioff(ipf,:)=duma
              pjoff(ipf,:)=dumb
            end do
      ...
      integer, intent(out) :: ier
      integer*2, dimension(pil*pjl*pnpan) :: ivar
      ...
      if (qtest) then
       ...
       if (size(var)/=pil*pjl*pnpan) then
        ...
        write(6,*) "size(var),pil,pjl,pnpan ",size(var),pil,
     &              pjl,pnpan
        call MPI_Abort(MPI_COMM_WORLD,-1,ierb)
       end if
      else
     ...
     &                               pncid(0),name,idv,ier
       ...
       if (qtest) then
        ...
        if (myid==0) then
         ...
         do jpf=1,jpmax-1
          call MPI_Recv(rvar,pil*pjl*pnpan,MPI_REAL,jpf,
     &            itag,MPI_COMM_WORLD,status,ierb)
          ip=ipf*nproc+jpf
          do n=0,pnpan-1
           no=n-pnoff(ip)+1
           ca=pioff(ip,no)
         ...
         call MPI_SSend(rvar,pil*pjl*pnpan,MPI_REAL,0,
     &           itag,MPI_COMM_WORLD,ierb)
        end if
       end if ! qtest
