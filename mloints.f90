...
interface mlob2ints
  module procedure mlob2ints_2, mlob2ints_3
end interface
...
!     (il+1,0),(il+2,0),(il+1,-1) (il+1,il+1),(il+2,il+1),(il+1,il+2)
    ...
    !$omp private(rmul_1,rmul_2,rmul_3,rmul_4,cmax,cmin,nn,np)
#endif
    do nn = 1,nlen
#ifdef _OPENMP
#ifdef GPU
...
if ( intsch==1 ) then
  ...
  sx(1:ipan,1:jpan,1:npan,1:wlev) = &
    reshape( s(1:ipan*jpan*npan,1:wlev), (/ ipan, jpan, npan, wlev /) )
  do k = 1,wlev
    do n = 1,npan
      do j = 1,jpan
        iq = 1+(j-1)*ipan+(n-1)*ipan*jpan
...
subroutine mlob2ints_3(s,nface,xg,yg,wtr,sal_test)
...
!     (il+1,0),(il+2,0),(il+1,-1) (il+1,il+1),(il+2,il+1),(il+1,il+2)
    ...
    !$omp parallel do collapse(2) schedule(static) private(k,iq,idel,xxg,jdel,yyg),          &
    !$omp private(n,cmul_1,cmul_2,cmul_3,cmul_4,dmul_2,dmul_3,emul_1,emul_2,emul_3,emul_4),  &
    !$omp private(rmul_1,rmul_2,rmul_3,rmul_4,nn,np)
#endif
    do nn = 1,nlen
#ifdef _OPENMP
#ifdef GPU
...
return
end subroutine mlob2ints_3

subroutine mlob2ints_2(s,nface,xg,yg,wtr,sal_test)
