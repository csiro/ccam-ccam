...
!        leaf area
  ...
  CASE(1)   ! dynamic allocation
    WHERE(casamet%iveg2/=icewater) 
      xLalloc(:) = min(1.0_r_2,max(0.0_r_2,exp(-0.5*casamet%glai(:))))   ! L limiting
      ! Pseudo-nutrient limitation calculation 
      WHERE(casamet%tsoilavg > 0.0) 
        xwsalloc(:) = min( max(casamet%moistavg(:)-real(soil%swilt(:),r_2),0.0_r_2) &
                         /real(soil%sfc(:)-soil%swilt(:),r_2), 1.0_r_2 )
      ELSEWHERE
        xwsalloc(:) = 0.01
      END WHERE
...
!    casaflux%crmplant(:,leaf) = casaflux%crmplant(:,leaf) + casaflux%clabloss(:)
    ...
    ! Growth efficiency correlated to leaf N:P ratio. Q.Zhang @ 22/02/2011
      casaflux%crgplant(:)  = (1.0-Ygrow(:))* max(0.0_r_2,casaflux%Cgpp(:)-SUM(casaflux%crmplant(:,:),2))
...
!                                 - casaflux%Nupland(nland)  
   ...
   IF(icycle >2) THEN
      ...
      fluxptase(nland) =  prodptase(veg%iveg(nland))*deltcasa  &
                       *  max(0.0_r_2,(casapool%Psoil(nland,2)+casapool%Psoil(nland,3))) &
                       *  max(0.0_r_2,(costNpup(veg%iveg(nland))-15.0))/(max(0.0_r_2,(costNpup(veg%iveg(nland))-15.0)) + 150.0)
      ...
      casapool%dPlitterdt(nland,:) = casaflux%fluxPtolitter(nland,:)  &
                                   - casaflux%klitter(nland,:)                 &
                                   * max(0.0_r_2,casapool%Plitter(nland,:))
...
!  print *, 'avgsoil: froot', veg%froot
  ...
  DO nland=1,mp
    ...
    casamet%btran(nland)     = casamet%btran(nland)+ veg%froot(nland,ns)  &
            * (min(real(soil%sfc(nland),r_2),casamet%moist(nland,ns))-real(soil%swilt(nland),r_2)) &
...
! Q.Zhang 23/05/2011 test code according to YPW
  WHERE(casamet%iveg2(:)/=icewater)
    WHERE((xFluxNsoilminnet(:)*deltpool + (casapool%Nsoilmin(:)-2.0)) > 0.0 &
      ...
      xkNlimiting(:) =1.0
    ELSEWHERE
      xkNlimiting(:) =MAX(0.0_r_2, - (casapool%Nsoilmin(:)-0.5) &
...
SUBROUTINE casa_Prequire(xpCnpp,Preqmin,Preqmax,PtransPtoP,veg, &
  ...
  WHERE(casamet%iveg2/=icewater)
    ...
    Preqmax(:,leaf)    = max(0.0_r_2,Preqmax(:,leaf) - PtransPtoP(:,leaf))
    Preqmax(:,wood)    = max(0.0_r_2,Preqmax(:,wood) - PtransPtoP(:,wood))
    Preqmax(:,froot)    = max(0.0_r_2,Preqmax(:,froot) - PtransPtoP(:,froot))

    Preqmin(:,leaf)    = max(0.0_r_2,Preqmin(:,leaf) - PtransPtoP(:,leaf))
    Preqmin(:,wood)    = max(0.0_r_2,Preqmin(:,wood) - PtransPtoP(:,wood))
    Preqmin(:,froot)    = max(0.0_r_2,Preqmin(:,froot) - PtransPtoP(:,froot))
  ENDWHERE
