...
C Argument list
      ...
      real prf(ln2,nl)
      real cdrop(ln2,nl)
      real ttg(ln2+iextra,nl)
      real qlg(ln2+iextra,nl)
      real qfg(ln2+iextra,nl)
      real qrg(ln2+iextra,nl)
      real precs(ln2)
      real qtg(ln2+iextra,nl)
      real cfrac(ln2,nl)
      real cffall(ln2+iextra,nl)
      real ccov(ln2,nl)
      real preci(ln2)
      real qevap(ln2,nl)
      real qsubl(ln2,nl)
...
c Remove small amounts of cloud
      ...
      do k=1,nl
        do mg=1,ln2
          if(qlg(mg,k)<1e-10.or.clfr(mg,k)<1e-5)then
            qtg(mg,k)=qtg(mg,k)+qlg(mg,k)
            ttg(mg,k)=ttg(mg,k)-hlcp*qlg(mg,k)
            qlg(mg,k)=0.
            clfr(mg,k)=0.
          endif
          if(qfg(mg,k)<1e-10.or.cifr(mg,k)<1e-5)then
            qtg(mg,k)=qtg(mg,k)+qfg(mg,k)
            ttg(mg,k)=ttg(mg,k)-hlscp*qfg(mg,k)
            qfg(mg,k)=0.
            cifr(mg,k)=0.
          endif
          if(qrg(mg,k)<1e-10.or.cffall(mg,k)<1e-5)then
