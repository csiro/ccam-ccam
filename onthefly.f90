...
if ( newfile ) then
  ...
  if ( zht_needed ) then
    if ( zht_found ) then  
      if ( tss_test .and. iop_test ) then
        ...
        call histrd(iarchi,ier,'zht',zss_a)
      else     
  ...
  if ( soilt_found ) then
    ...
    if ( .not.(tss_test.and.iop_test) ) then
      call histrd(iarchi,ier,'soilt',ucc)
      if ( fwsize>0 ) then
        nemi = 3
  ...
  if ( mlo_found ) then
    if ( tss_test .and. iop_test ) then
      ...
      call histrd(iarchi,ier,'ocndepth',ocndep_a)
    else     
  ...
  if ( nurban/=0 .and. nested/=1 .and. nested/=3 .and. .not.iop_test ) then
    ...
    if ( urban_found ) then
      call histrd(iarchi,ier,'t1_intmtgg1',ucc)  
    else if ( urban2_found ) then
      call histrd(iarchi,ier,'intmtgg1',ucc)
    else
      if ( fwsize>0 ) then
...
if ( nested==0 .or. (nested==1.and.retopo_test/=0) .or. nested==3 ) then
  if ( iop_test ) then
    call histrd(iarchi,ier,'psf',psl)
  else
    allocate( psl_a(fwsize) )
    psl_a(:) = 0.
    call histrd(iarchi,ier,'psf',psl_a)
  end if
endif
...
if ( tss_test .and. iop_test ) then
  call histrd(iarchi,ier,'tsu',tss)
  tss = abs(tss)
  ...
  call histrd(iarchi,ier,'tsu',tss_a)
  tss_a(:) = abs(tss_a(:))
  if ( fwsize>0 ) then
...
if ( tss_test .and. iop_test ) then
  ...
  call histrd(iarchi,ier,'siced',sicedep)
  call histrd(iarchi,ier,'fracice',fracice)
  ...
  call histrd(iarchi,ier,'siced',sicedep_a)
  call histrd(iarchi,ier,'fracice',fracice_a)
...
if ( nested/=1 .and. nested/=3 ) then
    ...
    do k = 1,ms 
      ...
      if ( iop_test ) then
        if ( k==1 .and. .not.tgg_found(1) ) then
          ...
          call histrd(iarchi,ier,vname,tgg(:,k))
        end if
      else
        if ( k==1 .and. .not.tgg_found(1) ) then
          ucc(1:fwsize) = tss_a(1:fwsize)
        else
          call histrd(iarchi,ier,vname,ucc)
        end if
  ...
  if ( all(wetfrac_found(1:ms)) ) then
    ...
    do k = 1,ms
      ...
      if ( iop_test ) then
        call histrd(iarchi,ier,vname,wb(:,k))
        if ( wetfrac_found(k) ) then
...
if ( iop_test ) then
  ...
  call histrd(iarchi,ier,vname,varout)
else
...
if ( iop_test ) then
  ! read without interpolation or redistribution
  call histrd(iarchi,ier,uname,uarout)
  call histrd(iarchi,ier,vname,varout)
else
  ! for multiple input files
  call histrd(iarchi,ier,uname,ucc)
  call histrd(iarchi,ier,vname,vcc)
...
if ( iop_test ) then
  ! read without interpolation or redistribution
  call histrd(iarchi,ier,vname,u_k)
else
  ! for multiple input files
  call histrd(iarchi,ier,vname,ucc)
...
if ( iop_test ) then
  ! read without interpolation or redistribution
  call histrd(iarchi,ier,uname,u_k)
  call histrd(iarchi,ier,vname,v_k)
else
  ! for multiple input files
  call histrd(iarchi,ier,uname,ucc)
  call histrd(iarchi,ier,vname,vcc)
