...
!            & MIN(2._r_2, rat) ) ! 0 for -ve rat
        ...
        IF (ntest > 0) THEN
          ...
          PRINT * , 'speeda,speedb,fluxhi,fluxlo,delt,rat,phi ', &
             & speed_k(idjd), 0.5*soil%zse(k)/dt, fluxhi(idjd), &
        ...
        speed_k = MIN(speed_k, 0.5 * soil%zse(k) / dt)
      ...
      WHERE( ssoil%wb(:,ms) > soil%sfc(:))
        ...
        fluxlo = wbl_k
        ! scale speed to grid lengths per dt & limit speed for stability
        speed_k = MIN(speed_k, 0.5 * soil%zse(ms) / dt)
...
!        PRINT * , 'pwb_wbh,pwb_min* for ms ', pwb_wbh(idjd), &
!                                 & soil%hsbh(idjd) * pwb_min(idjd)
...
!                        * ssoil%smass(:,3)/ssoil%osnowd
  ...
  !-------------------------------------------------------------------------
  SUBROUTINE surfbv (dt, ktau, met, ssoil, soil )
    REAL(r_1), INTENT(IN)                    :: dt   ! integration time step (s)
    INTEGER(i_d), INTENT(IN)                 :: ktau ! integration step number
    TYPE(met_type), INTENT(INOUT)            :: met    ! all met forcing
    TYPE(soil_snow_type), INTENT(INOUT)      :: ssoil  ! soil+snow variables
...
!    canopy%sghflux = coefa * (ssoil%tggsn(:,1) - ssoil%tggsn(:,2) )
  ...
  !	 ccnsw - soil thermal conductivity, including water/ice
  !
  SUBROUTINE stempvsn(dt,soilcond, soilhcp, canopy, ssoil, soil)
    REAL(r_1), INTENT(IN)                    :: dt ! integration time step (s)
...
!    print *,'stempvsn ga',canopy%ga
  ...
  SUBROUTINE snowcheck(dt, ktau, ssoil )
    ...
    IF (ntest > 0) THEN
      ...
      PRINT *, 'dt,ssdn ', dt, (ssoil%ssdn(idjd,k),k=1,3)
...
!        ssoil%smass(:,3) = ssoil%sdepth(:,3) * ssoil%ssdn(:,3)
  ...
  !******************************************************************
  SUBROUTINE snowl_adjust(dt, ktau,  ssoil, canopy )
    REAL(r_1), INTENT(IN)               :: dt ! integration time step (s)
    INTEGER(i_d), INTENT(IN)            :: ktau ! integration step number
    TYPE(soil_snow_type), INTENT(INOUT) :: ssoil
    TYPE(canopy_type), INTENT(INOUT)    :: canopy
...
!          ssoil%wb(:,k) = ssoil%wb(:,k) -   evapfbl(:,k)
  ...
  SUBROUTINE soil_snow(dt, ktau, soil, ssoil, veg, canopy, met, bal)
    REAL(r_1), INTENT(IN)                    :: dt ! integration time step (s)
    INTEGER(i_d), INTENT(IN)                 :: ktau ! integration step number
    TYPE(soil_parameter_type), INTENT(INOUT) :: soil
    TYPE(soil_snow_type), INTENT(INOUT)      :: ssoil
...
!    REAL(r_2), DIMENSION(mp):: sicemelt
    ...
    CALL snowl_adjust(dt, ktau, ssoil, canopy )
...
!    ssoil%rnof1 = MAX(0., totwet - dt / 172.8) ! 86400/500 = 172.8
    ...
    CALL surfbv(dt, ktau,  met, ssoil, soil )
