...
namelist/cardin/comment,dt,ntau,nwt,npa,npb,nhorps,nperavg,ia,ib, &
    ...
    procformat,procmode,compression,cns_node_reader,              & ! file io
...
if ( myid == 0 ) then
  ...
  do nproc_high = nproc,10*nproc
    call proctest_face(npanels,il_g,nproc_high,nxpb_test,nyp)
    if ( nxpb_test > 0 ) exit
  end do
  nxpb = nxpb_test
  do nproc_low = nproc,1,-1
  ...
  do nproc_high = nproc,10*nproc
    call proctest_uniform(npanels,il_g,nproc_high,nxpb_test,nyp)
    if ( nxpb_test > 0 ) exit
  end do
  if ( nxpb>0 ) then
    if ( nxpb_test>0 ) then
  ...
  do ilg_low = il_g,1,-1
    call proctest_face(npanels,ilg_low,nproc,nxpa_test,nyp)
    if ( nxpa_test > 0 ) exit
  end do
  nxpa = nxpa_test
  do ilg_high = il_g,10*il_g
    call proctest_face(npanels,ilg_high,nproc,nxpb_test,nyp)
    if ( nxpb_test > 0 ) exit
  end do
  nxpb = nxpb_test
  do ilg_low = il_g,1,-1
    call proctest_uniform(npanels,ilg_low,nproc,nxpa_test,nyp)
    if ( nxpa_test > 0 ) exit
  end do
  if ( nxpa>0 ) then
    if ( nxpa_test>0 ) then
  ...
  do ilg_high = il_g,10*il_g
    call proctest_uniform(npanels,ilg_high,nproc,nxpb_test,nyp)
    if ( nxpb_test > 0 ) exit
  end do
  if ( nxpb>0 ) then
    if ( nxpb_test>0 ) then
