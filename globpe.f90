...
read(99, cardin)
call ccmpi_bcast(kdate_s,0,comm_world)
call ccmpi_bcast(ktime_s,0,comm_world)
call ccmpi_bcast(ntau,0,comm_world)
read(99, skyin)
read(99, datafile)
...
do ktau = 1,ntau   ! ****** start of main time loop
  ...
  if ( abs(iaero)>=2 ) then
    call aerocalc
    xtg(1:ifull,1:kl,1:naero) = min( xtg(1:ifull,1:kl,1:naero), 6.4e-6 )
  end if
...
if ( mod(nproc,6)/=0 .and. mod(6,nproc)/=0 ) then
  ...
  ! search for valid process decomposition.  CCAM enforces the same grid size on
  ! each process
...
nyp = nproc/nxp                ! number of processes in Y direction
! search for vaild process decomposition.  CCAM enforces the same grid size on
! each process
