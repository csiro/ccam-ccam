...
do k=1,kl
  ...
  prf(:,k) = b_ps(tile)%data(:)*sig(k)
end do

! Tracer settling
call trsettling(rhoa,b_t(tile)%data(:,:),dz,prf,tile,imax)
...
!   set it to zero
  ...
  call gasvmix(updtr,gasfact,igas,decay,trsrc,methloss,mcfloss,b_cdtq(tile)%data(:),dz(:,1),tile,imax)
  
  call trimt(at,ct,updtr,imax)
  b_tr(tile)%data(:,:,igas) = updtr  
...
select case(trim(tractype(igas)))
  ...
  case('online')
    if (trim(tracname(igas)(1:3)).eq.'cbm') then
      select case (trim(tracname(igas)))
        case('cbmnep'); trsrc(:,1) = b_fnee(tile)%data(:)
        case('cbmpn');  trsrc(:,1) = b_fpn(tile)%data(:)
        case('cbmrp');  trsrc(:,1) = b_frp(tile)%data(:)
        case('cbmrs');  trsrc(:,1) = b_frs(tile)%data(:)
      ...
      if (mveg<1.or.mveg>maxval(b_ivegt(tile)%data(:))) stop 'tracer selection: veg type out of range'
  ...
  case ('daypulseon')
    ...
    if (tracdaytime(igas,1)<tracdaytime(igas,2) .and. tracdaytime(igas,1)<=timeg .and. tracdaytime(igas,2)>=timeg) then
      trsrc(:,1) = b_co2em(tile)%data(:,igas)
  ...
  case default
    ...
    do k=1,traclevel(igas)
      trsrc(:,k) = b_co2em(tile)%data(:,igas)/real(traclevel(igas))
    end do
...
if (methloss) then
  loss = b_tr(tile)%data(:,:,igas)*dt*(koh*exp(-1775./b_t(tile)%data(:,:))*b_oh(tile)%data(:,:) + b_strloss(tile)%data(:,:))

!       calculate total loss
  totloss_l(1) = 0.
  do k=1,kl
    do iq=1,imax
      totloss_l(1) = totloss_l(1) + loss(iq,k)*dsig(k)*b_ps(tile)%data(iq)*b_wts(tile)%data(iq)
    enddo
  enddo
#if 0
...
!         accumulate loss over month
  ...
  dep = 0.
elseif (mcfloss) then
  loss = b_tr(tile)%data(:,:,igas)*dt*(kohmcf*exp(-1520./b_t(tile)%data(:,:))*b_oh(tile)%data(:,:) + b_jmcf(tile)%data(:,:))
  ! deposition
  dep  = exp(-b_mcfdep(tile)%data(:)*dt/dz1)
...
temptr(:,1)   = b_tr(tile)%data(:,1,igas)*drate*dep - fluxfact*grav*dt*trsrc(:,1)/(dsig(1)*b_ps(tile)%data(:)) - loss(:,1)
do k=2,kl
  temptr(:,k) = b_tr(tile)%data(:,k,igas)*drate - fluxfact*grav*dt*trsrc(:,k)/(dsig(k)*b_ps(tile)%data(:)) - loss(:,k)
end do
...
do nt = 1, ngas
  ...
  if ( trden(nt)>0. .and. trreff(nt)>0. ) then
    ...
    newtr = b_tr(tile)%data(:,kl,nt)*exp(-b)
    newtr = max( newtr, 0. )
    dfall = max( b_tr(tile)%data(:,kl,nt) - newtr, 0. )
    b_tr(tile)%data(:,kl,nt) = newtr
    ! Solve each vertical layer successively (layer l)
    do k = kl-1,1,-1
      ! Update mixing ratio
      ...
      ! Fout  = 1.-exp(-b)
      ! Fthru = 1.-Fout/b
      newtr = b_tr(tile)%data(:,k,nt)*exp(-b) + dfall*(1.-exp(-b))/b
      newtr = max( newtr, 0. )
      dfall = max( b_tr(tile)%data(:,k,nt) + dfall - newtr, 0. )
      b_tr(tile)%data(:,k,nt) = newtr
    end do
  end if
end do
