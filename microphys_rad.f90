...
!--------------------------------------------------------------------
      if (Lw_control%do_lwcldemiss_iz) then
        ...
        write(6,*) "ERROR: Lw_control%do_lwcldemiss has not been initialized"
...
!--------------------------------------------------------------------
      ...
      if (Cldrad_control%do_strat_clouds_iz) then     
        ...
        write(6,*) "ERROR: Cldrad_control%do_strat_clouds has not been initialized"
        !call error_mesg ('microphys_rad_mod', &
...
!--------------------------------------------------------------------
      if (Cldrad_control%do_sw_micro_iz) then
        ...
        write(6,*) "ERROR: Cldrad_control%do_sw_micro has not been initialized"
        !call error_mesg ('microphys_rad_mod', &
...
!    present.
!---------------------------------------------------------------------
      if (present(Lscrad_props) .or. present(Lsc_microphys))  then     
        if (.not. present(Lscrad_props) .or. .not. present(Lsc_microphys))  then
          !call error_mesg ('microphys_rad_mod', &
...
subroutine cloudpar (nonly, nbmax, nnn, size_drop, size_ice, size_rain, & 
                     ...
                     isccp_call, cldext, cldsct, cldasymm)
...
!--------------------------------------------------------------------
      ...
      integer  :: nb
      integer  :: i,j,k
      real, dimension(size(conc_drop,1),size(conc_drop,2),size(conc_drop,3)) :: &
          sum, sum2, sum3
      ...
      do ni=nistart, niend
        where (mask)
          cldextivlliq(:,:,:,ni) = 1.0E+03*conc_drop*        &
                                   (1.0E-02*a(ni) + (b(ni)/  &
                                   (0.5*size_drop)   ) )
          cldssalbivlliq(:,:,:,ni) = 1.0 - ( c(ni) + d(ni)*  &
                                     0.5*size_drop )
          cldasymmivlliq(:,:,:,ni) = e(ni) + 1.0E-03*f(ni)*  &
                                     0.5*size_drop
        end where
      end do
...
! </SUBROUTINE>
!
subroutine fu (conc_ice, size_ice, cldextivlice,  &
...
!---------------------------------------------------------------------
      ...
      else if ((.not. do_delta_adj) .and. do_const_asy) then
        do ni = nistart, niend
          where (mask)
      ...
      else if (do_delta_adj .and. do_const_asy) then
        fd =  1.1572963e-1 +                       &
              2.5648064e-4*size_ice +              &
              1.9131293e-6*size_ice2               &
             -1.2460341e-8*size_ice3
...
!----------------------------------------------------------------------
      if (isccp_call) then
        do n=1,Cldrad_control%nlwcldb
! MJT suggestion - I think el_dge should only be called for Donner option
!          if (nbmax == 1) then
...
! </SUBROUTINE>
!
subroutine el  (nb, conc_ice, size_ice, mask,  &
...
!---------------------------------------------------------------------
      ...
      ! MJT suggestion - precalculate integrated terms
      sumext = 0.
      sumssalb = 0.
      sumasymm = 0.
!-----------------------------------------------------------------------
